...
const lines = [
  ...
  "",
  ".env.release.local Handoff",
  "If owners prefer a file-based handoff, copy the approved session values below into .env.release.local. Release summary commands read .env.release.local automatically; explicit shell environment variables still override file values.",
  "Do not store secrets, database passwords, DATABASE_URL, API tokens, or raw credentials in .env.release.local.",
  ...Object.entries(approvedValues).map(([key, value]) => `${key}=${value}`),
  "LOCAL_ENV_FILES=.env.release.local pnpm release:summary-preflight",
  "LOCAL_ENV_FILES=.env.release.local pnpm release:summary",
  "",
  ...
  "",
  "Final Gate Reminder",
  "Before the final manifest, run DATABASE_URL=<pilot-or-staging-url> pnpm release:readiness-register after Admin > Release Readiness gates, evidence records, and Release Board decisions are current.",
  "Before the final manifest, rerun pilot readiness with DATABASE_URL=<pilot-or-staging-url> PILOT_REQUIRE_RELEASE_GATES_READY=true pnpm release:pilot-readiness.",
  "The strict pilot readiness artifact must include requireReleaseGatesReady=true and DEC-0036 strict release gate status.",
  "Run pnpm release:evidence:manifest only after final source evidence, signed documents, and external-security proof references are collected.",
