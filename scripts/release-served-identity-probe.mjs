...
const baseUrl = process.env.RELEASE_PUBLIC_BASE_URL;
...
if (!baseUrl?.startsWith("https://") || !/^[a-f0-9]{40}$/.test(commitSha ?? "") || !/^[a-f0-9]{64}$/.test(artifactSha256 ?? "") || !/^[A-Za-z0-9_-]{16,128}$/.test(fence ?? "")) {
...
if (response.status !== 200 || response.headers.get("cache-control") !== "no-store, max-age=0" || response.headers.get("x-ogfi-proxy-fence") !== fence) {
  throw new Error("RELEASE_SERVED_IDENTITY_RESPONSE_INVALID");
}
const identity = await response.json();
if (identity?.commitSha !== commitSha || identity?.artifactSha256 !== artifactSha256 || identity?.probe !== probe) {
  throw new Error("RELEASE_SERVED_IDENTITY_MISMATCH");
}
console.log(`RESULT | PASS | served release identity verified for ${url.hostname}`);
