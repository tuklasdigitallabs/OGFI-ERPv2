...
const lines = [
  ...
  "",
  ".env.release.local",
  "Copy the Template Values into .env.release.local when the release owner approves them. Release summary commands read .env.release.local automatically; explicit shell environment variables still override file values.",
  "Do not store secrets, database passwords, DATABASE_URL, API tokens, or raw credentials in .env.release.local.",
  ...Object.entries(templateValues).map(([key, value]) => `${key}=${value}`),
  "",
  "Command-scoped local env file",
