...
function isPrivilegedMfaEnforcementMode(
  ...
  return (
    typeof value === "string" &&
    privilegedMfaEnforcementModes.includes(value as PrivilegedMfaEnforcementMode)
  );
}
...
async function getPrivilegedMfaEnforcementMode(session: SessionContext) {
  ...
  if (saved?.status === "ACTIVE" && isPrivilegedMfaEnforcementMode(saved.value)) {
    return saved.value;
  }
...
export async function hasVerifiedPrivilegedMfaEvidence(session: SessionContext) {
...
export async function assertPrivilegedMfaForAction(
  ...
  if (input.permissionCode && !isSensitivePermissionCode(input.permissionCode)) {
    return {
      required: false,
