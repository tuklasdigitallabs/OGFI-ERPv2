...
export function GoodsReceiptLinesEditor({ action, orders }: Props) {
  ...
  return <form action={action} className="flex min-h-[34rem] flex-col" onSubmit={submit}>
    ...
    <div className="sticky bottom-0 z-10 shrink-0 border-t border-slate-200 bg-white/95 px-4 py-3 text-right shadow-[0_-8px_20px_rgba(15,23,42,0.06)] backdrop-blur"><button className="inline-flex min-h-11 w-full items-center justify-center rounded-md bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700 sm:w-auto">Create Draft Receipt</button></div>
  </form>;
}
