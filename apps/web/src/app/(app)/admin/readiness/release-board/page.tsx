...
async function recordDecisionAction(formData: FormData) {
  ...
  try {
    ...
    redirect(actionErrorRedirectPath("/admin/readiness/release-board", error));
  }
  redirect("/admin/readiness/release-board");
}
  ...
  return (
    <AppShell session={session} title="Release Board" subtitle="Decision and release blocker review" activeNav="admin-readiness">
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[var(--shadow-surface)]">
        <form method="get" className="grid gap-3 border-b border-slate-100 bg-slate-50 p-4 md:grid-cols-[1fr_12rem_auto] md:items-end"><label className="grid gap-1 text-sm font-medium text-slate-700">Search note/reference<input name="q" defaultValue={query} className="min-h-11 rounded-md border border-slate-300 bg-white px-3" /></label><label className="grid gap-1 text-sm font-medium text-slate-700">Decision<select name="decision" defaultValue={decision ?? ""} className="min-h-11 rounded-md border border-slate-300 bg-white px-3"><option value="">All decisions</option>{releaseBoardDecisions.map((value) => <option key={value} value={value}>{decisionLabel(value)}</option>)}</select></label><button type="submit" className="min-h-11 rounded-md bg-blue-600 px-4 text-sm font-semibold text-white">Apply</button></form>
