...
export default async function AdminReadinessPage({
  ...
  let gates: Awaited<ReturnType<typeof listReleaseReadinessGates>>;
  try {
    gates = await listReleaseReadinessGates(session);
  } catch {
    return (
      <AppShell
        session={session}
      ...
      >
        <Panel className="border-amber-200 bg-amber-50 text-amber-950">
          <h2 className="text-lg font-bold">Release Readiness is unavailable</h2>
          <p className="mt-2 text-sm">This workspace requires Core Administration permission and active Manage scope for the selected company. Ask an authorized administrator to assign the scope, then retry.</p>
        </Panel>
      </AppShell>
    );
  }
          ...
          <Panel className="mb-5 border-blue-100 bg-blue-50/40">
            {selectedDeploymentEvidence ? (
              <div className="grid gap-3 text-sm text-slate-700">
                ...
                {selectedDeploymentEvidence.verificationStatus === "RECORDED" ? (selectedDeploymentEvidence.createdByUserId === session.user.id ? <p className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 font-semibold text-amber-900">Another authorized reviewer must review evidence recorded by you.</p> : <TaskSheet title="Review selected deployment evidence" description="Choose one audited outcome. The server rechecks company scope, creator self-review, and RECORDED status before changing the record." trigger="Open review controls" triggerClassName="min-h-11 bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700" bodyScroll="contained"><div className="grid gap-3" data-testid="deployment-evidence-review-controls"><div className="rounded-xl border border-blue-100 bg-blue-50/70 p-4 text-sm text-slate-700"><p className="font-bold text-slate-950">{selectedDeploymentEvidence.title}</p><p className="mt-1">Current status: {selectedDeploymentEvidence.verificationStatus}. Verify confirms the evidence; reject records a reason and keeps it out of deployment readiness counts.</p></div><form action={updateDeploymentEvidenceAction}><input name="evidenceId" type="hidden" value={selectedDeploymentEvidence.id} /><input name="status" type="hidden" value="VERIFIED" /><input name="reason" type="hidden" value="Verified deployment evidence from the selected evidence review controls." /><input name="deploymentQ" type="hidden" value={deploymentQ} /><input name="deploymentType" type="hidden" value={deploymentEvidenceType ?? ""} /><input name="deploymentStatus" type="hidden" value={deploymentVerificationStatus ?? ""} /><input name="deploymentEnvironment" type="hidden" value={deploymentEnvironment} /><input name="deploymentPage" type="hidden" value={String(deploymentEvidencePage.page)} /><input name="deploymentPageSize" type="hidden" value={String(deploymentEvidencePage.pageSize)} /><input name="deploymentEvidenceId" type="hidden" value={selectedDeploymentEvidence.id} /><button className="min-h-11 w-full rounded-md border border-emerald-200 bg-emerald-50 px-3 text-sm font-semibold text-emerald-800">Verify evidence</button></form><form action={updateDeploymentEvidenceAction} className="grid gap-2"><input name="evidenceId" type="hidden" value={selectedDeploymentEvidence.id} /><input name="status" type="hidden" value="REJECTED" /><input name="deploymentQ" type="hidden" value={deploymentQ} /><input name="deploymentType" type="hidden" value={deploymentEvidenceType ?? ""} /><input name="deploymentStatus" type="hidden" value={deploymentVerificationStatus ?? ""} /><input name="deploymentEnvironment" type="hidden" value={deploymentEnvironment} /><input name="deploymentPage" type="hidden" value={String(deploymentEvidencePage.page)} /><input name="deploymentPageSize" type="hidden" value={String(deploymentEvidencePage.pageSize)} /><input name="deploymentEvidenceId" type="hidden" value={selectedDeploymentEvidence.id} /><label className="grid gap-1 text-sm font-medium text-slate-700">Rejection reason<input name="reason" className="min-h-11 rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="Explain why this evidence is not accepted" minLength={5} required /></label><button className="min-h-11 rounded-md border border-rose-200 bg-rose-50 px-3 text-sm font-semibold text-rose-800">Reject evidence</button></form></div></TaskSheet>) : null}
