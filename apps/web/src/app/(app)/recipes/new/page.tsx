...
const inputClass =
  "min-h-10 rounded-lg border border-slate-300 bg-white px-3 py-2";
  ...
  try {
    ...
    const brandQuery =
      typeof brandId === "string" && brandId
        ...
        : "";
    redirect(`/recipes/${recipeId}${brandQuery}`);
  } catch (error) {
    const brandId = formData.get("brandId");
    const returnPath =
      typeof brandId === "string" && brandId
        ? `/recipes/new?brandId=${encodeURIComponent(brandId)}`
        : "/recipes/new";
...
export default async function CreateRecipePage({
  ...
  const requestedBrandParam = resolvedSearchParams.brandId;
  const requestedBrandId = Array.isArray(requestedBrandParam)
    ? requestedBrandParam[0]
    : requestedBrandParam;
  const manageableBrands = brandScope.brands.filter((brand) => brand.canManage);
  const defaultBrandId = manageableBrands.some(
    (brand) => brand.id === session.context.brandId,
  )
    ? session.context.brandId
    : (manageableBrands[0]?.id ?? null);
  const selectedBrandKey =
    requestedBrandId === "COMPANY_SHARED" && brandScope.canManageCompanyShared
      ? "COMPANY_SHARED"
      : manageableBrands.some((brand) => brand.id === requestedBrandId)
        ? requestedBrandId!
        : (defaultBrandId ??
  ...
  if (!selectedBrandKey) {
    redirect("/recipes?error=MENU_RECIPE_BRAND_SCOPE_DENIED");
  }
  const options = await getRecipeCreateOptions(session, {
  ...
  const selectedBrandName =
    selectedBrandKey === "COMPANY_SHARED"
      ? "Company-shared"
      : (manageableBrands.find((brand) => brand.id === selectedBrandId)?.name ??
        "Unavailable");
  const recipesHref = `/recipes?brandId=${encodeURIComponent(selectedBrandKey)}`;
  ...
  return (
    ...
    >
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <ButtonLink href={recipesHref} tone="ghost" className="ogfi-chip">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Badge tone="info">Brand: {selectedBrandName}</Badge>
          <Badge tone="neutral">
            Preview location: {session.context.locationName}
          </Badge>
        </div>
      </div>
      ...
      <form action={createDraftRecipeAction} className="grid gap-5">
          ...
          <div className="ogfi-section-header">
            <div>
              ...
              <p className="text-sm text-slate-500">
                The current Company and Brand are fixed when this draft is
                submitted. Location is operational preview context and does not
                own the recipe.
              </p>
            </div>
            ...
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              ...
              >
                ...
                {brandScope.canManageCompanyShared ? (
                  <option value="COMPANY_SHARED">Company-shared recipe</option>
                ) : null}
              </select>
            </label>
            <p className="mt-2 text-xs font-semibold text-slate-500">
              This owning brand is stored with the recipe. Active brands come
              from your Company or Brand scope; a branch location is not
              required.
            </p>
          </div>
          <div className="grid gap-4 p-4 md:grid-cols-3">
            ...
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              ...
              <input
                ...
                placeholder="Kitchen, Commissary, R&D"
              />
            </label>
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              Target food cost %
        ...
        <section className="ogfi-data-surface overflow-hidden">
          <div className="ogfi-section-header">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Define the controlled costing basis for the initial draft.
              </p>
            </div>
          </div>
        ...
        <RecipeIngredientLinesEditor
          items={options.items.map((item) => ({
            id: item.id,
            code: item.code,
            name: item.name,
          }))}
          uoms={options.uoms.map((uom) => ({ id: uom.id, code: uom.code }))}
          defaultUomId={firstUomId}
        />
        ...
        <section className="ogfi-data-surface p-4">
          <label className="grid gap-1 text-sm font-semibold text-slate-700">
            ...
            <textarea
              ...
              name="notes"
              placeholder="Test kitchen context or serving assumptions"
            />
          </label>
        </section>
        ...
        <div className="sticky bottom-0 z-10 -mx-2 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-white/95 px-4 py-3 shadow-[0_-8px_20px_rgba(15,23,42,0.06)] backdrop-blur">
          <p className="text-xs font-semibold text-slate-500">
            Creates a draft only; inventory, POS, finance, and menu prices are
            unchanged.
          </p>
          <div className="flex gap-2">
            <ButtonLink href="/recipes" tone="ghost">
              Cancel
            </ButtonLink>
            <button
              className="inline-flex min-h-11 items-center justify-center rounded-md bg-blue-600 px-5 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
              type="submit"
            >
              Create Draft Recipe
            </button>
