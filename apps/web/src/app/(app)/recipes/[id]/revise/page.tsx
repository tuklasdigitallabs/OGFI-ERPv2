...
export default async function ReviseRecipePage({
  params,
  searchParams,
}: {
  ...
  return (
    <AppShell
      session={session}
      title={`Revise ${recipe.recipeName}`}
      subtitle="Focused draft revision task"
      activeNav="recipes"
    >
      ...
      <form action={createRecipeRevisionDraftAction} className="grid gap-5">
          ...
          <div className="ogfi-section-header">
            <div>
              ...
              <p className="text-sm text-slate-500">
                A new draft is created; the published costing basis remains
                unchanged.
              </p>
            </div>
          ...
          <div className="grid gap-4 p-4 md:grid-cols-2 lg:grid-cols-5">
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              ...
              <input
                ...
                min="0.000001"
                step="0.000001"
                defaultValue={recipe.yieldQuantity ?? undefined}
                required
              />
            </label>
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              ...
              <select
                className={inputClass}
                name="yieldUomId"
                defaultValue={recipe.yieldUomId ?? undefined}
                required
              >
            ...
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              ...
              <input
                ...
                min="0.000001"
                step="0.000001"
                defaultValue={recipe.servingQuantity ?? undefined}
                required
              />
            </label>
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              ...
              <select
                className={inputClass}
                name="servingUomId"
                defaultValue={recipe.servingUomId ?? undefined}
                required
              >
                {options.uoms.map((uom) => (
                  <option key={uom.id} value={uom.id}>
            ...
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              ...
              <input
                ...
                max="100"
                step="0.01"
                defaultValue={recipe.targetFoodCostPercent ?? undefined}
              />
            </label>
          </div>
          <div className="border-t border-slate-100 p-4">
            <label className="grid gap-1 text-sm font-semibold text-slate-700">
              ...
              <input
                className={inputClass}
                name="reason"
                placeholder="Supplier yield, portion standard, costing refresh..."
                required
              />
            </label>
          </div>
        ...
        <section className="ogfi-data-surface overflow-hidden">
          <div className="ogfi-section-header">
            <div>
              <h2 className="text-lg font-bold text-slate-950">
                Current ingredient lines
              </h2>
              <p className="text-sm text-slate-500">
                Adjust quantities, preparation notes, order, or mark a line for
                removal.
              </p>
            </div>
          ...
          <div className="grid gap-3 p-4">
            {recipe.lines.map((line) => (
              <fieldset
                key={line.id}
                className="grid gap-3 rounded-xl border border-slate-200 p-3 md:grid-cols-[5rem_1.3fr_8rem_1fr_7rem] md:items-end"
              >
                <legend className="sr-only">
                  Current ingredient {line.itemName}
                </legend>
                <label className="grid gap-1 text-xs font-bold uppercase text-slate-500">
                  Order
                  <input
                    className={inputClass}
                    name={`line.${line.lineNo}.sortOrder`}
                    type="number"
                    min="1"
                    max="100"
                    step="1"
                ...
                <label className="grid gap-1 text-xs font-bold uppercase text-slate-500">
                  ...
                  <input
                    className={inputClass}
                    name={`line.${line.lineNo}.quantity`}
                    type="number"
                    min="0.000001"
                    step="0.000001"
                ...
                <label className="grid gap-1 text-xs font-bold uppercase text-slate-500">
                  ...
                  <input
                    className={inputClass}
                    name={`line.${line.lineNo}.preparationNote`}
                    defaultValue={line.preparationNote ?? ""}
                    placeholder="Optional"
                  />
                </label>
                <label className="flex min-h-10 items-center gap-2 rounded-lg border border-rose-200 px-3 text-sm font-semibold text-rose-700">
                  <input
                    name={`line.${line.lineNo}.remove`}
                    type="checkbox"
                    value="1"
                  />
                  Remove
        ...
        <RecipeRevisionAddLinesEditor
          items={options.items}
          uoms={options.uoms}
          subRecipes={options.subRecipes}
          defaultUomId={firstUomId}
          firstSortOrder={recipe.lines.length + 1}
        />
        ...
        <section className="ogfi-data-surface p-4">
          <label className="grid gap-1 text-sm font-semibold text-slate-700">
            ...
            <textarea
              ...
              name="notes"
              placeholder="Internal costing notes for this draft version"
            />
          </label>
        </section>
        ...
        <div className="sticky bottom-0 z-10 -mx-2 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-white/95 px-4 py-3 shadow-[0_-8px_20px_rgba(15,23,42,0.06)] backdrop-blur">
          <p className="text-xs font-semibold text-slate-500">
            Published recipe, menu price, inventory, POS, and finance records
            remain unchanged.
          </p>
          <div className="flex gap-2">
            <ButtonLink href={`/recipes/${recipe.id}`} tone="ghost">
              Cancel
            </ButtonLink>
            <button
              className="inline-flex min-h-11 items-center justify-center rounded-md bg-blue-600 px-5 text-sm font-bold text-white hover:bg-blue-700"
              type="submit"
            >
