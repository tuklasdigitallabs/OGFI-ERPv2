...
import { buildBankCashExportRows } from "@/server/services/finance";
...
export async function GET() {
  ...
  try {
    ...
    const rows = await buildBankCashExportRows(session);
    await logOperationalExportAudit({
      session,
      reportId: "phase-3-bank-cash",
    ...
    return csvExportResponse(rows, "phase-3-bank-cash.csv", {
