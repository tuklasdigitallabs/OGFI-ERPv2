...
import { receiveInventoryTransfer } from "@/server/services/transfers";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ status: "error", message: getActionFeedback({ error: code })?.message ?? "The receipt was not posted. Review the form and try again." }, { status: 400 });
  }
