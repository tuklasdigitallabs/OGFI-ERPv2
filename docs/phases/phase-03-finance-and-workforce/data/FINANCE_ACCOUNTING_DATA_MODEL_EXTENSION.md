...
| `supplier_credit_notes` | Supplier credits/refunds. | supplier_id, original_invoice_id, reference, amount, status |
...
| `cash_advances` / `liquidations` | Extend existing Phase III entities. | finance posting state, journal source references, settlement state |

### 2.5 Budget, assets, projects, AR, and allocations

| Entity | Purpose | Key fields |
|---|---|---|
| `budgets` / `budget_lines` | Extend existing finance framework. | dimensions, account/category, period, original/revised/committed/actual values |
| `budget_commitments` | Tracks approved obligations. | source document/event, budget line, amount, status |
