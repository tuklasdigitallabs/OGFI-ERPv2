...
- Stock Balances uses server-side pagination with a bounded 10-row page, deterministic tie-breaking, query-aware counts, and URL-backed `All`, `Positive stock`, and `Expiring soon` filters. The `Balance rows` summary is the exact matching total for the current search and tab, not the visible page length. Updated dates render in the selected company's configured timezone; missing or invalid configuration safely falls back to the current operational default, `Asia/Manila`. Ordinary CSV export remains a separate full-result operation for the current authorized location and search.
- `positive-stock-v1` is the closed Overview destination for current positive balance rows. It fixes tenant, company, selected location, active Inventory Location, and `qtyOnHand > 0` on the server; only bounded search may narrow it. The profile preserves search and paging, provides an explicit route back to all balances, and identifies current live data rather than claiming a historical dashboard snapshot. Its CSV export uses the same predicate and deterministic order, enforces `reporting.export.max_rows` before and after its bounded fetch, and returns a truthful `413` instead of a partial file when the synchronous limit is exceeded.
- `zero-stock-v1` is the closed Overview destination for current zero-stock balance rows. It fixes tenant, company, selected location, active Inventory Location, exact relation ownership, and `qtyOnHand = 0` on the server. Positive and negative rows, inactive or malformed relations, cross-scope rows, and catalog items without an existing balance row are excluded. Multiple valid zero rows for one item remain separate balance rows. The surface must say `Zero-stock balance rows`, not `Items configured`; it is neither catalog completeness nor automatic replenishment.
- `lot-expiry-data-v1` is the closed Overview destination for existing balance rows whose trimmed lot is nonblank or expiry date is recorded. It includes positive, zero, and negative quantities and excludes blank or whitespace-only lot when expiry is absent. Tenant, company, selected location, active Inventory Location, and Location/Item/Category/base-UOM ownership are fixed by one static parameterized server predicate used by count, page, and export. The profile label is `Rows with lot or expiry data`; absent lot or expiry displays `Not recorded`, negative quantities retain distinct warning styling, and visible copy states that data presence is not compliance, coverage, accountability, traceability, or a historical snapshot.
- The Inventory profile dispatcher is exhaustive for `positive-stock-v1`, `zero-stock-v1`, and `lot-expiry-data-v1`. Profile requests accept exact-cardinality `dashboard`, optional bounded search, and optional valid page only; ordinary tab, scope, and filter overrides fail visibly. All profiles are current live, read-only views, preserve page/search/profile context through independently authorized ledger navigation, and use deterministic ordering ending in balance ID.
- Zero-stock profile CSV uses the exact dashboard/list/page predicate and order, independently requires export authority, applies `reporting.export.max_rows` through count preflight and a `maxRows + 1` guard, and returns safe `413` with no partial file. Its audit metadata is aggregate-only and excludes search text, row payloads, identifiers, and internal errors.
- Lot/expiry-data profile CSV uses the exact static dashboard/list/page predicate and deterministic order, independently requires export authority, applies `reporting.export.max_rows` through count preflight and a `maxRows + 1` guard, and returns safe `413` with no partial file. Audit metadata remains aggregate-only. Future `postInventoryMovement` writes normalize lot text to trimmed text or null for movement and balance records; legacy rows are not rewritten and blank-lot data quality remains a separate follow-up.
