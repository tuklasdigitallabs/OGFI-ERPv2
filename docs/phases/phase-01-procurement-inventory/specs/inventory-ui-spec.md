...
- The Inventory profile dispatcher is exhaustive for `positive-stock-v1` and `zero-stock-v1`. Profile requests accept exact-cardinality `dashboard`, optional bounded search, and optional valid page only; ordinary tab, scope, and filter overrides fail visibly. Both profiles are current live, read-only views, preserve page/search/profile context through independently authorized ledger navigation, and use deterministic ordering ending in balance ID.
