...
- Inventory dashboard-profile dispatch is exhaustive for `positive-stock-v1` and `zero-stock-v1`. Profile mode accepts exactly one `dashboard`, optional single normalized search of at most 120 characters, and optional single valid page; raw tab/scope/filter overrides fail visibly. Both profiles show current live rows rather than historical snapshots, preserve canonical context through independently authorized ledger navigation, and retain existing balance/export permissions.
