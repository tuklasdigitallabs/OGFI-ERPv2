...
| Role | KPI cards |
|---|---|
| Executive / GM | Pending approvals by age, open PO count and overdue follow-up, critical stock locations, count/variance exceptions |
...
- `DEC-0235` suppresses `PO commitment`, `Open PO exposure`, and `Received value`. Broad all-status header/line rollups and newest-PO currency formatting are not authoritative monetary reconciliation and have no closed destination parity. Overview preserves the exact non-monetary `po-open-v1` count/profile and overdue PO queue. The Reports tab exposes only a neutral `Purchase Order Register` source-workspace link with record/status/currency wording; it makes no aggregate-value or reconciliation claim. No replacement monetary metric is approved.
- Inventory balance visibility does not establish a trusted inventory valuation. Overview therefore suppresses any `Inventory value` availability card until costing basis, currency, reconciliation, and an exact authorized destination are approved.
- Under implemented `DEC-0238`, the compatible dashboard `reports` route/view is visibly labeled `Source views`. It is a typed operational destination directory, not a generic report surface: `Exact operational views` are versioned, read-only scoped populations, while `Source workspaces` are authorized modules without an implied record population, aggregate, reconciliation, or availability claim.
- A Source views entry is enrolled only when its authorized dashboard source was attempted. The entry labels that source available or unavailable without treating an unavailable source as zero records, complete data, or a successful workspace query. It separates Wastage Exceptions, Stock Adjustment Exceptions, Branch Checklist Exceptions, Open Incidents, and Maintenance Follow-up rather than using broad combined reporting labels. Exact destinations use their existing server-owned profile href helpers; workspace destinations independently authorize. The directory has a visible no-destination state and performs no workflow or export action.
...
- The Overview `Reporting trust gate` is a read-only policy-control status, not a KPI or source-data availability claim. Its policy read, observation, and exact `ledger-variance-v1` destination are enrolled only for users with both balance-view and ledger-view permissions. Unentitled users receive no trust mode, override, decision identifier, policy failure, observation, or reconciliation link. The status label wraps on narrow screens and never presents `Source pending`, `Open source`, or a configuration action.
...
- Overview suppresses the former `Updated this week` / recent-stock-updates signal under `DEC-0234`. Mutable `InventoryBalance.updatedAt` is cache lifecycle metadata, not authoritative inventory-event time, and generic `/inventory` is not a closed event drilldown. No replacement card or candidate list is shown. A future immutable Inventory Movement profile remains unapproved pending timestamp, period/week/timezone, grain, and ledger-permission decisions.
...
- Under implemented `DEC-0236`, the Branch Operations `Critical exception lines` widget opens only the read-only `branch-checklist-critical-exceptions-v1` profile. Its primary metric is the exact count of retained lines with `result = EXCEPTION` and `severity = CRITICAL` across every checklist status. The destination reports the critical-line total and affected-checklist count separately. It is an all-status severity lens, not an open-work or actor-actionable count.
- Overview does not expose standalone `Manager Review` or `Reviewed` signals. The former duplicates the combined Reviews profile and `waiting` conflicts with started-review semantics; the latter is an intermediate state that excludes `CLOSED` and has no confirmed period, owner, or close-policy definition. The combined Reviews card and `branch-checklist-reviews-v1` profile remain authoritative.
- Branch Operations profiles accept only bounded search and server-owned pagination as additional narrowing controls. Raw status, shift, business date, and scope inputs cannot widen or redefine them. Invalid or stale profile identifiers fail visibly, row/detail navigation preserves the profiled return context, and source actions reauthorize independently. Profile presentation uses cards below the `lg` breakpoint, shows a selected-brand badge when applicable, and labels the reviewer field `Reviewed by`.
...
- Under implemented `DEC-0237`, the Food Safety `Critical exception readings` widget opens the read-only `food-safety-critical-exceptions-v1` destination. Its primary metric is the exact count of retained readings with `result = EXCEPTION` and `severity = CRITICAL` across every Food Safety log status. The destination reports that reading total and the affected-log count separately. It is an all-status severity lens, not an open-work or actor-actionable count.
- Overview suppresses standalone `Exception Review` and `Reviewed` Food Safety signals. The former duplicates the combined Reviews profile and `waiting` conflicts with started-review semantics; the latter is an intermediate state that excludes `CLOSED` and has no confirmed period, owner, or close-policy definition. The combined Reviews card and `food-safety-reviews-v1` profile remain authoritative.
- The critical profile accepts only bounded, normalized search and server-owned pagination as additional narrowing conditions. Raw type, status, business date, and scope cannot widen or redefine it. Invalid, duplicate, empty, stale, or unknown profile identifiers fail visibly; create and ordinary export are hidden; record navigation retains only canonical profile/search/page return context; and source actions reauthorize independently. Parent-log and child-reading tenant, company, optional-brand, and location scope are relation-safe wherever readings contribute to membership or display.
