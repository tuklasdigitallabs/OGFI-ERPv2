...
Implemented ordinary-register filter note: the selected-location Receiving Reports register supports server-owned exact Supplier, Purchase Order, and Receiver filters. The same filters are preserved through tab links, pagination, and ordinary CSV export. Option lists are derived from the scoped receipt population, capped at 100 recent receipts, include a selected value when present, and disclose when the cap is reached; historical inactive receivers are labeled `(inactive)`. The existing GRN/PO/supplier search can narrow the option population. Item and accepted-value filters remain deferred until their relation/query-plan and cost visibility/currency/null-value decisions are approved and evidenced.
