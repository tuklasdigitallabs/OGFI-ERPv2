...
- After those locks, both receipt posting and reversal revalidate the live active user/session privilege epoch, action permission, exact destination-location scope, and local MFA freshness inside the transaction; a stale or revoked authority context fails closed before any source, movement, balance, rollup, or audit mutation.
