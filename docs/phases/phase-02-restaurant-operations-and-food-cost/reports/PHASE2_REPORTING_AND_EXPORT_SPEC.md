...
- The Food Safety `food-safety-critical-exceptions-v1` dashboard destination counts retained `EXCEPTION` + `CRITICAL` readings across every log status under exact relation-safe parent-log and child-reading tenant, company, optional-brand, and location scope. It reports the exact critical-reading total and distinct affected-log count separately, supports only bounded narrowing search and server-owned pagination, and grants no workflow or export authority.
- Food Safety reporting retains the combined `SUBMITTED` + `EXCEPTION_REVIEW` Reviews profile and the all-severity Exceptions profile, while suppressing standalone Exception Review and Reviewed dashboard signals. `EXCEPTION_REVIEW` is already included in Reviews, and `REVIEWED` excludes `CLOSED` so it is not a completion metric without a separately confirmed period, owner, and close-policy definition.
