...
**Last verified against:** Implemented `Food Safety Exceptions` and `Food Safety Reviews` dashboard profiles
...
`Operations Dashboard` → `Food Safety Exceptions` or `Food Safety Reviews`
...
3. Select `Food Safety Reviews` to open every scoped log currently in `SUBMITTED` or `EXCEPTION REVIEW`. This view is not limited to records assigned to you.
4. Use Search to narrow the active profile. Clear Search to restore its complete server-defined population.
5. Open a log to inspect its current readings, corrective-action context, evidence references, and activity where available.
6. Use the preserved return link to go back to the same dashboard profile, search, and page.
7. Exit the dashboard view when you need the ordinary Food Safety workspace and its normal filters or authorized controls.
...
- Both profiles are read-only oversight views. Opening a profile or source log does not grant review, correction, or closure authority.
...
- Dashboard summaries and the authorized CSV export are separate views from the ordinary register and from the two read-only dashboard profiles.
