...
10. A request creates an immutable command; it does not itself perform the action. In an admitted local-UAT environment, the isolated worker passes only that command ID to the separately credentialed opening-stock executor. Check the `Activity` tab for the command lifecycle, completion time, or safe failure code. Do not submit a second request while the matching command is pending, claimed, or retrying.
...
- If the local-UAT worker is stopped or unavailable, the immutable command remains pending and stock remains unchanged. Contact the UAT owner to restore the reviewed worker; never run a migration credential, direct SQL update, balance edit, or general runtime process as a substitute.
