...
7. Open `Readiness` and select `Validate readiness`. Confirm that the workspace evaluates exactly these eight families: `PurchaseRequest`, `QuotationRecommendation`, `PurchaseOrder`, `InventoryTransfer`, `StockCountAttemptReview`, `WastageReport`, `StockAdjustment`, and `OpeningInventoryCutover`. All family rows start closed and show their status, blocker count, and evidence cutoff. Select one row to inspect its blockers, select it again to close it, or select another family to switch the detail.
8. Review every row marked `Blocked` and expand it for the bounded blocker list. Correct the displayed issue in the authoritative source record, update the draft input when required, and run `Validate readiness` again. Do not treat `Ready at cutoff` as a permanent grant.
