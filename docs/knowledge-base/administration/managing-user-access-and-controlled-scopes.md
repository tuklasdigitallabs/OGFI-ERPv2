...
- Audit Trail also supports an exact UUID Entity ID filter. Supplier Audit links use Admin → Audit Trail with the selected Supplier entity filter preserved through paging, export, and event detail; the server rechecks company scope and does not widen a stale or foreign ID into a broader search.
...
- Opening a Location Context shows an assignment-grain Assigned Access register, not a complete user directory. It lists active, currently effective assignments for active users in the current tenant, reports the exact request-time assignment total, and paginates 10–100 rows. The page is read-only; use User Access to grant or revoke scope. Duplicate active assignments, if present, are shown as separate assignments and the ordering is deterministic.
- Opening a Company Context shows a read-only, assignment-grain Company Access register. It lists active, currently effective company assignments for active users in the current tenant, supports bounded name/email search and 10–100 row pagination, and reports the exact request-time assignment total. Brands and Locations are summary counts; use Organization Scope for their full registries, and use User Access for access changes.
- Approval Rule detail shows two read-only paginated registers: the ordered approval-step chain and selected-company Related Audit Activity. Totals are exact, pages are bounded, and inactive or unavailable assignee references are labeled rather than silently omitted. Use the Approval Rules workspace and existing controlled actions for policy changes; viewing a rule does not grant approval authority.
- Audit Event detail is an inspection-only, tenant/scope-authorized view. Sensitive values remain redacted, and very large or deeply nested payloads show an explicit partial-display notice; the immutable audit event is not changed. Timestamps are shown in the event company timezone.
- Role Detail’s Assigned Users panel shows currently effective assignments only. Scope badges are selected-company previews capped at eight per user; they are context, not a complete access grant. Use User Access and the source workspace for authoritative access and scope decisions.
...
- `Create Role` closes only after the server confirms creation and then shows a success toast. New roles start without permissions. In Role Detail, permission choices are retained in browser memory while you move between permission pages or filters; select `Save Permission Overrides` to commit them. `Cancel and Return` or a full browser refresh discards the unsaved choices. A failed save or failed recommended-set action keeps the unsaved choices available for correction.
...
Permission Access detail is read-only. It shows tenant-local and tenant-global roles that grant one permission, labels each role as Global or Tenant, and provides bounded previews of active users who currently have effective scope in the selected company (up to five previews per granting role). Expand a role's preview disclosure only when needed; previews are limited to the current granting-role page and are not an exhaustive effective-user total. It does not grant or change access; use the Roles & Permissions controls for authorized mutations. Search and page the granting-role list when a permission has many grants.
...
The Overview Effective Permissions panel is a bounded preview derived from
currently effective active roles and assignments (including start/end dates).
When you need the complete union for this user, open the Roles section's
read-only effective-permission register. It supports server-side search and
pagination, shows exact matching totals, and keeps the selected user and
company context visible. Select a permission label to open the read-only
Permission Access detail route for its definition and granting-role context;
neither surface changes access.
