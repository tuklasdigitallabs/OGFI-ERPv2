...
**Last verified against:** implemented action-first Operations Dashboard checkpoint, closed Open Purchase Orders drilldown, Approval Inbox, project My Work, and scoped in-app notifications
...
4. Select `Open` or `Open approvals` to continue in the relevant controlled workspace. The `Open POs` card opens the selected location's closed Open Purchase Orders view.
...
- A dashboard drilldown narrows an already authorized source list. It does not grant access to a PO, alter the selected scope, or permit changing the dashboard filter through the URL.
