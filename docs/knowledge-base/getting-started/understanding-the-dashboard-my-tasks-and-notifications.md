...
2. Open `Operations Dashboard`. Its Overview presents up to four authorized summary rows in this order: `Assigned Approvals`, `Operational Exceptions`, `Priority Indicators`, and `Stock Balance Signals`. All rows start closed. Select a row to expand its details below the summary, select it again to close it, or select another row to switch details.
...
5. Expand `Priority Indicators` after reviewing approval and exception summaries. The dashboard places positive authorized exception counts before zero-value indicators, then uses the combined permissions of your effective roles to prioritize up to six relevant records. This changes presentation only and never grants access. Open `Source views` for every other authorized destination in the selected scope.
...
19. Open `Notifications` to review unread or actionable alerts. The Notification Center remains the primary surface. If your role permits manual scans, a closed `Manual reminder scans` summary appears below the feed and shows how many scan families are available and whether this view has a result.
20. Expand `Manual reminder scans` only when you need a scan utility. If you are an approver, use `Scan Approvals` to create current-user reminders for eligible due or overdue approvals. While Approval Inbox is unavailable, this scan is only a partial reminder mechanism: its links cannot open an approval action and it does not provide a complete queue.
21. If you are an authorized project manager, use `Scan Reminders` inside the same summary to create in-app reminders for due or overdue project tasks. Authorized restaurant-operations users may run the separate exception scan shown there. Selecting the summary again closes it.
22. Mark notifications read or archive them after handling the related source record.

## Expected Result

- The compact dashboard header shows the selected scope, role label, response assembly time, source status, and view tabs. Overview uses full-width summary rows instead of a card grid. Every closed row shows up to three compact, source-backed snapshot values, and at most one detail panel is open at a time.
- A positive authorized exception is not hidden merely because a role-preferred indicator is zero. Role-aware ordering uses the user's effective permission capabilities and does not broaden source enrollment or record access.
...
- The approval and exception rows state how many records are currently shown without adding the two different record populations together. Open the source workspace for the authoritative record and complete list.
...
- When the dashboard says the approval preview is unavailable, do not infer a zero-approval result. `Approval Inbox` is also unavailable while normalized routing is disabled and exposes no complete queue or action path. The ordinary local development server on `http://localhost:3001` intentionally remains outside the hardened bounded-approval admission contract; changing a local feature flag does not safely activate approvals there. `Scan Approvals` may create current-user reminders for eligible due or overdue work, but those links remain unavailable and the scan does not disclose every pending approval. Follow the workflow owner for release guidance; there is no hidden legacy action queue and the unavailable state does not change who can approve a record.
