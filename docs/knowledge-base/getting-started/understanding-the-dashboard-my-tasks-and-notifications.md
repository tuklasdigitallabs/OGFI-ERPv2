...
- My Tasks currently exposes an enrolled-module filter. It narrows the already authorized source adapters and preserves the filter while paging. It does not expose arbitrary location, assigned-by, priority, due-date, or cross-module status filters yet. Do not infer those meanings from a task's creator, reporter, opener, submitter, or displayed location text; the remaining controls require every enrolled adapter to apply the same server-owned predicates to both counts and pages.
