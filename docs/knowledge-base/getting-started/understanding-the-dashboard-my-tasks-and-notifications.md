...
- My Tasks currently exposes enrolled-module, priority, and source-qualified status filters. They narrow the already authorized source adapters and remain bound while paging. It does not expose arbitrary location, assigned-by, or native due-date filters yet. Do not infer those meanings from a task's creator, reporter, opener, submitter, or displayed location text; the remaining controls require explicit authority and source semantics.
