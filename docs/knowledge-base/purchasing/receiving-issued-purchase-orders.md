...
1. Open `Receiving`.
   The ordinary register is server-paginated. Use the `All`, `Draft`, `Posted`, or `Discrepancies` tabs, search by GRN, Purchase Order reference, or supplier name, and optionally filter by supplier, Purchase Order, one of the listed receipt statuses, or a received-date range to narrow the current authorized location; dates are interpreted as calendar days in OGFI’s initial operational timezone (`Asia/Manila`). Supplier and Purchase Order choices come from a bounded recent receipt population and may require a narrower search. Page changes do not load the full receipt history into the browser.
