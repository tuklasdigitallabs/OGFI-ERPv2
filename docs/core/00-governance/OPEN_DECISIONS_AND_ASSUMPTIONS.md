...
10. Production evidence architecture is confirmed by `DEC-0045`: AWS S3 with GuardDuty Malware Protection behind provider-neutral adapters; one private protected bucket per environment; opaque quarantine keys and immutable exact object versions; GuardDuty clean tag plus PostgreSQL clean/available state; non-authoritative EventBridge/API Destination wake-up with exact-version revalidation and idempotent compare-and-set; bounded database-backed reconciliation; S3 Versioning, SSE-KMS, Object Lock Governance, and cross-account replication/backup. Production activation still requires approved AWS account/Region/data residency, KMS/IAM/GuardDuty/backup/recovery/incident owners, operating budget and alarms, retention classes and legal-hold/governance-bypass authority, file and storage/scan quotas, RPO/RTO, recovery procedures, and hosted staging/restore proof. Object Lock Compliance mode remains prohibited without explicit Legal approval. Evidence capture remains hybrid: structured notes or verified external references plus private uploads where policy requires files; text alone does not satisfy high-risk evidence unless explicitly allowed by policy.
