...
- Parent confirmation, 2026-07-27: the Overview `Reporting trust gate` is governed by the same dual-permission boundary as the `ledger-variance-v1` profile. The dashboard may read and present the company trust posture only for users with both `inventory.balance.view` and `inventory.ledger.view`; other users receive no trust metadata, source observation, or reconciliation destination. This clarification changes disclosure/entitlement parity only and does not change the configured policy value or reconciliation query.
