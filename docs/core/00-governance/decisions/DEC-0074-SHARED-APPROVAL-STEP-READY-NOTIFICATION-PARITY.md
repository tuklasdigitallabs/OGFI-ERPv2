...
It is written in the same transaction as the current-step decision and next-step activation and may create at most one personal notification under retry. A role-scoped next step creates zero per-user notification rows and is designed for discovery through live Approval Inbox eligibility after controlled activation. `APPROVAL_ROUTING_V1_ENABLED` remains `false`.

That eligibility contract does not provide a complete actionable role-scoped queue
while the flag is false. The public Approval Inbox fails closed and shows no legacy
or parallel list. The existing permissioned `Scan Approvals` reminder tool may create
current-user reminders for eligible due or overdue work, but it is partial and its
approval links remain unavailable; authoritative actionable discovery begins only
after controlled normalized-Inbox activation.
...
| Complete legacy initial procurement notification parity as a separate checkpoint | Product Governance + Engineering | Before normalized-routing activation | Five source paths implemented by `DEC-0050` / `DEC-0078`; three source-serialization PostgreSQL specifications authored, execution pending |
...
- Direct-user activation passes the exact prepared recipient and emits at most one personal row under the deterministic key. The helper is not called for role-scoped activation, preserving zero role-member fanout. Live eligibility is the normalized Inbox's future authority, but the flag-off public Inbox exposes no role-scoped work. The notification metadata includes approval instance ID, exact activated-step ID/order, `DIRECT_USER`, assigned user, assigned role value, permission code, and location-context scope.
...
- The disposable PostgreSQL command fails only with `DISPOSABLE_DATABASE_ADMIN_URL_REQUIRED`. Final independent Architecture review reports no Critical/High finding and returns GO only for the feature-disabled source-control checkpoint. Security reports no Critical/High issue introduced by this change and keeps activation at NO-GO; it requires executable PostgreSQL before accepting this checkpoint as behavioral evidence. The later five initial-procurement paths are implemented under `DEC-0050` and `DEC-0078`, and the three exact source-serialization race specifications are authored but unexecuted. Broader prohibited-actor/live-revocation, visible role-scoped Inbox activation, all-family parity, unresolved policy, cutover/drain, exact-candidate authorization/E2E, and hosted release/recovery gates remain open.
- Source-truth assessment found no new visible behavior, approval policy, permission, action, status, label, or navigation change while normalized routing remains disabled. The knowledge base and glossary are nevertheless corrected in the `DEC-0078` reconciliation checkpoint so they do not claim the normalized Inbox is currently live; no training or user-facing release-note behavior change is required.
...
- `docs/core/07-quality/CURRENT_PENDING_IMPLEMENTATION_PLAN.md` records the implemented five-path initial notification contract, the three authored but unexecuted source-serialization races, and visible role-scoped discovery as remaining activation evidence.

## Supersession

This decision is not superseded. It applies the notification contract of `DEC-0050` to the seven-family shared current-step advance path without replacing broader approval-integrity, source-locking, prohibited-actor, policy, or activation gates. `DEC-0078` later implements source serialization for the five initial procurement paths; its three exact PostgreSQL race specifications are authored but unexecuted and do not activate a visible role-scoped Inbox. A later decision that changes direct versus role-scoped recipient behavior, deterministic event-key semantics, metadata authority, transaction placement, or activation posture must explicitly amend or supersede this record.
