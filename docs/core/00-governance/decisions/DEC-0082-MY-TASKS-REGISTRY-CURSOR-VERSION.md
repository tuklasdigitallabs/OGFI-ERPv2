...
The signed My Tasks cursor scope hash includes an explicit registered-source contract version. The current value is `my-tasks-registry-v6`. It must be bumped whenever an enrolled adapter’s eligibility predicate, source enrollment, ordering semantics, task projection, or queue filter contract changes.
...
`v6` invalidates earlier cursors after correcting Branch Operations and Food
Safety status-filter intersection for actors who hold only review or only create
authority. It changes no source action permission or mutation behavior.
