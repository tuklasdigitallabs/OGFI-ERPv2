...
|---|---|---|---|
| Implement and review unconditional protected-table guards with preservation preflight and migration recovery notes | Database Engineering | Before SPF006-IMM-01 hosted verification | Implemented, locally rehearsed on PostgreSQL 17.10, and independently `APPROVED_FOR_REHEARSAL` at the exact migration hash |
| Implement the non-login owner, controlled migrator, and restricted runtime role split; remove transitive elevation paths | Database Engineering / Platform Engineering / Security | Before production promotion | Implemented for Hostinger and locally adversarially verified; hosted credential, systemd, network, and effective-role proof remains pending |
| Convert ordinary seed behavior to additive/idempotent operations and disposable demo reset to positive-identity database rebuild | Backend Engineering / Database Engineering | Before hosted verification | Implemented and locally verified; exact hosted demo-reset execution remains pending |
| Give automated tests a unique disposable-database lifecycle with administrative credentials unavailable to the application-under-test | QA / Platform Engineering / Database Engineering | Before hosted verification | Implemented with per-run roles, pre-mutation identity binding, and positive database identity; local lifecycle passes 11/11 and exact hosted execution remains pending |
...
- The exact guard migration hash `52cccdce8596f9d1b287bc771c219ccf82cbe8b5c43f438ce407ba08bd671efa` is independently database-reviewed as `APPROVED_FOR_REHEARSAL`. PostgreSQL 17.10 evidence passes 17/17 real guard cases with stable non-empty protected-table snapshots, matched inventory movement/balance totals, atomic lock-timeout rollback, idempotent redeploy, and zero Prisma schema drift.
- Local lifecycle and role-contract evidence passes 11/11 disposable-lifecycle tests and 8/8 database-role tool tests. Controlled and adversarial identities are bound to the expected per-run database before mutation, and cross-run identity or role substitutions are rejected. Eight adversarial drift cases covering roles, ownership, grants, the guard function, and triggers each failed closed, were repaired through the controlled path, and left zero per-run roles after teardown; the PostgreSQL 17.10 mismatch-rejection rehearsal likewise left zero adversarial roles. Independent database/security review returned GO for this local checkpoint.
- The complete local candidate passes 27 database tests, 823 web tests, one worker test, lint, typecheck, production build, release-tool self-tests, secret review, migration review in release mode, and diff validation. This does not satisfy the hosted production gate.
