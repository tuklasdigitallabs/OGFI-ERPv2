...
- Runtime request paths must use a tenant/company-scoped structural check on the caller's existing transaction client. The exhaustive operator cutover inspector remains outside workflow transactions; invoking it while holding action locks can self-block. Run the steady-state structural check before decision mutation, never during the intentional current-step-to-next-step transition.
- For role-assigned next steps, select one deterministic eligible anchor actor before mutation, include that actor with the current actor in canonical sorted `User` locks, and revalidate that exact actor after the approval/current/next-step locks. Do not silently switch to an unlocked actor after lock acquisition.
...
| Implement dynamic paginated inbox eligibility and action-time revalidation for role, permission, effective dates, scope, step state, and no-self-approval | Backend Engineering + Frontend Engineering + Security | Before SPF-006 local checkpoint | Inbox and shared-engine Purchase Request evidence implemented behind disabled cutover flag; canonical specialized/Finance Close preflight and all-18 action matrix pending |
...
- A focused Architecture/Backend/Database/Security/QA review of action readiness split exhaustive operator certification from a supplied-transaction runtime structural gate. Fresh PostgreSQL evidence passes 23/23 backfill, inbox, and representative Purchase Request action cases, including malformed sources, live revocation, terminal races, action/dry-run coexistence, and a two-step transition with a distinct deterministic next-role anchor. Review keeps activation at NO-GO until the ten specialized handlers and Finance Close use the same canonical pre-mutation authority/session/next-actor lock contract and the heterogeneous all-18 action matrix passes.
