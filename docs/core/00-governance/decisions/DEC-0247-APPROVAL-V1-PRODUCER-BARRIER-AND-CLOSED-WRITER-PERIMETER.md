...
The next procurement decision increment hardens Quotation Recommendation final
approval after terminal return/reject. It reuses the recommendation lineage
locks and validates the selected Supplier Quotation before CASing
`PENDING_APPROVAL` to `APPROVED` with the locked version, quotation-request
linkage, and `updatedAt`. No Purchase Order, supplier master, receiving,
inventory, payment, or journal mutation is introduced. Stock Adjustment
...
### Purchase Order first issuance correction (July 27, 2026)
...
### Budget Revision terminal/approval legacy-parity correction (July 27, 2026)

Architecture, Security, and QA accepted a source-only BudgetRevision perimeter correction. Both approval and rejection now share the barrier → approval graph → revision → parent Budget → ordered BudgetLine/location scope lock order, exact `UNDER_REVIEW`/`updatedAt`/lineage checks, live scope and requester SOD, and source CAS. Approval remains request-only; rejection only terminalizes the revision and graph. Budget lines, balances, commitments, payments, AP, bank, journals, periods, and inventory are not mutated. PostgreSQL race, rollback, ACL/SOD, and zero-side-effect evidence remain open.
...
### Inventory Transfer dispatch source/line/ledger fencing correction — July 27, 2026

Architecture and Security accepted the bounded dispatch correction. The writer preserves source-only `TRANSFER_OUT` posting, locks inventory locations before the scoped transfer header and ordered lines, rehydrates active location/line facts, CASes `REQUESTED → DISPATCHED` against the locked `updatedAt`, uses deterministic `dispatch:<lineId>` movement keys, CASes line totals, and audits after all writes. Exact retries succeed only when the complete movement/line state is present; inconsistent dispatch residue fails closed. Destination receipt and all AP/payment/bank/journal/budget/approval effects remain separate. PostgreSQL contention, duplicate exactly-once, rollback, scope, and no-destination-effect evidence remain open.
