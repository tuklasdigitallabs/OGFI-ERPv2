...
## 2026-07-24 — Supplier Quote Commercial Components

- Added confirmed `DEC-0105`: supplier quotes now persist server-calculated Decimal subtotal, tax, discount, freight, other-charge, and supplier-accreditation snapshot values. Existing totals are preserved through an additive backfill; binary quote attachments remain separately deferred pending the controlled-evidence requirement matrix.
