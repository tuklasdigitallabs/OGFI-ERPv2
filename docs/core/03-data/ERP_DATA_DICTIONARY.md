...
| Transfer Receipt      | `inventory_transfer_id`, `received_by_user_id`, `status`, `received_at`, `idempotency_key`, `idempotency_request_hash`, `posted_at`, `reversed_by_user_id`, `reversed_at`, `reversal_reason`, `discrepancy_flag`, `discrepancy_summary`                                                                                                                  | Durable receipt event for exact or partial destination receipt. New receipt callers will bind a bounded tenant/company-scoped idempotency key and canonical SHA-256 request hash; legacy rows may remain null. Same-key/same-hash replay is permitted only after live actor/location/transfer authorization; changed payload or scope fails closed. Posted receipt events can be reversed only as full events. |
