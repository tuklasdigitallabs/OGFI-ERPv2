...
Current implementation note: shared `Attachment` metadata stores tenant, private storage provider, internal object key, original filename, MIME type, size, checksum, uploader, status, and creation timestamp. Phase 3 controlled evidence uses `ControlledEvidenceAttachment` as the company-scoped source-record link for finance/workforce evidence. Private binary upload writes files under the configured local-private storage root, records checksum, enforces the evidence-storage policy limit and MIME allowlist, and writes audit metadata for storage policy, scan-waiver mode, retention, recovery, and download-audit requirements. Download is server-mediated, source-permission checked, checksum verified, and audited. Production still requires storage-provider, scan/scan-waiver, retention/recovery, and UAT signoff before binary evidence is represented as production-ready.
