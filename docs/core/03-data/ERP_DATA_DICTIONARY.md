...
| Permission            | `permission_key`, `module`, `action`                                                  | Includes `core.tenant_role_administer` for tenant role administration and sensitive `purchasing.supplier_confidential.view` as additional Supplier commercial-data clearance; other examples include `purchase_request.submit`, `purchase_order.approve`, and `purchase_order.issue`. |
...
| `AuthLoginAttempt` | keyed tenant/account/source digests, attempt type, optional challenge-session ID, outcome, timestamp | Legacy operational throttle history retained only for bounded cleanup and compatibility; new password/MFA admission does not create a per-attempt row. |
| `AuthenticationThrottleWindow` | attempt type, fixed dimension type, server HMAC bucket key and version, optional fixed shard number, optional resolved tenant/account, UTC window, snapshotted limit, exact request/failure-reservation/success/denied counters, threshold time, retention time | Bounded pre-verification throttle state. Unknown identifiers and sources create only fixed shard rows; tenant/account dimensions require real tenant-qualified foreign keys. A successful verification converts exactly one conservative failure reservation to success. |
| `AuthenticationThrottleControl` | singleton ID, ACTIVE/PAUSED status, generation, active key version/fingerprint/from time, optional previous generation/version/fingerprint/retire time, policy digest, timestamps | PostgreSQL is authoritative for the accepted throttle key/policy generation. Runtime reservations take a reviewed shared lock; only the controlled migrator operator function may pause, resume, or rotate through expected-generation compare-and-swap. Secret key material remains outside PostgreSQL. |
...
Authentication throttling uses one reservation call after optional identity resolution, so global and shard dimensions are never double-counted. Per attempt type and UTC window, unknown-input cardinality is bounded by `1 + identifier_shard_count + source_shard_count`; resolved rows add at most one row per real tenant and one row per real account. Defaults use 64 identifier shards, 64 source shards, 15-minute windows, and 30-day retention, with startup validation limiting shards to 16–1024, windows to 1–60 minutes, retention to 1–365 days, and counters/limits to 1–1,000,000. Exact source-address rate limiting remains an edge/WAF responsibility.

The HMAC key version is immutable per window. A domain-separated signed reservation binds its random ID, exact sorted dimensions and row IDs, tenant/account, database window, generation, key fingerprint, policy digest, and attempt type. Completion requires the matching immutable successful-authentication `AuditEvent`, consumes the reservation exactly once, and uses the same canonical dimension lock order as reservation. The singleton control row rejects null status, stale generation, active/previous key identity reuse, and rotation while the prior generation remains unexpired. Bounded cleanup and health jobs are implemented; production activation still requires Hostinger calibration, external alert delivery, and restore evidence.

#### Authorization Denial Bucket (`DEC-0050`)

`AuthorizationDenialBucket` is mutable operational aggregation state, not a replacement for immutable audit history. Required fields are tenant, optional company/location, actor or a bounded non-actor subject enum, closed normalized action/reason/resource enums, a server-derived bucket key, 5-to-60-minute window bounds, exact count, first/last denial timestamps, first immutable audit-event link, optional final-summary audit-event link, finalization timestamp, and lifecycle timestamps. Company, location, and actor relationships are tenant-qualified. A unique tenant/key/window constraint permits exactly one bucket per bounded dimension window. The first denial event commits atomically with bucket creation; an exact final summary is created once only when the count exceeds one. Buckets and linked audit history are never deleted, truncated, or backfilled from synthetic estimates.

The configured `security.authorization_denial.window_minutes` company policy defaults to `15` and accepts integer values from `5` through `60`. Bucket dimensions never include attacker-selected target IDs, record references, paths, query or payload values, filenames, raw network addresses, error messages, stack traces, or database errors.
...
Current Phase I scaffold implements active supplier-to-item purchase UOM links, optional supplier SKU/name, lead time, minimum order quantity, preferred rank, optional reference unit price, deactivation with reason, and audit history. Under `DEC-0242`, the latest reference unit price, currency, and effective metadata are confidential commercial data and must be omitted from unauthorized query/service projections; nonblank writes require the separate confidential clearance plus existing management authority. Implementation evidence for this boundary remains pending. It does not yet make these links eligible for quote comparison, purchase orders, receiving, or supplier exception approval.
...
| Entity            | Required fields                                                                                                                 | Notes                                                                                                                |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Approval Instance | `tenant_id`, `company_id`, `transaction_type`, `transaction_id`, `approval_policy_id`, `current_step_no`, `approval_status`, `snapshot_json` | Snapshot protects historical rule integrity. PostgreSQL permits at most one `PENDING` instance for a tenant/company/document-type/document-ID tuple. Pre-existing duplicates block migration and require authorized, manual, audited reconciliation; they are never automatically deleted, merged, or rewritten. |
...
### 7.4 Normalized Approval-Step Eligibility (`DEC-0050`)

`ApprovalInstanceStep` stores immutable routing context after `routing_schema_version` becomes `1`: exactly one assigned user or role, the required permission relation, `ALL` scope-group matching, activation timestamp, optional due timestamp, and delegation provenance. Scope requirements are relational rather than JSON or arrays. Every scope group has one or more targets and uses `ANY` or `ALL` target matching; all groups must pass. Targets identify a company and, where applicable, a brand or location in the approval instance's tenant/company. Prohibited actors are stored per step with a bounded reason code.

Active user, effective active role assignment, live permission grant, active scope assignment with `APPROVE` or `MANAGE`, active target resources, tenant/company identity, current-step status, and prohibited-actor exclusion are evaluated at read and action time. Direct-user routing does not bypass permission or scope checks. Initial and later step activation writes exactly one immutable `approval.step_activated` audit event in the workflow transaction. Normalized reads remain disabled until an idempotent document-type backfill reports zero incomplete `PENDING` or `WAITING` routing contexts.

The normalized Approval Inbox reads the same materialized eligibility predicate for rows and total count with bounded server pagination. The UI feature remains disabled by `APPROVAL_ROUTING_V1_ENABLED=false` until executable coverage proves all 18 registered document types, direct and role assignment, `ANY`/`ALL` groups, revocation/effective dates, no-self-approval, due-soon boundaries, detail hydration, concurrent action, and rollback comparison.

### 7.5 Durable Approval-Routing Backfill Orchestration (`DEC-0245`)

| Entity | Required fields | Integrity and lifecycle notes |
|---|---|---|
| Approval Routing Backfill Run | `tenant_id`, `company_id`, `mode`, `status`, routing schema/mapping version and hash, capability version and hash, `release_identity`, start request and idempotency identity, request hash, database lease owner/expiry, fencing token, pass, `(created_at,id)` cursor, next batch sequence, receipt head, cumulative counts, optional `stop_audit_event_id`, timestamps | Only durable `APPLY` creates a run; `DRY_RUN` is read-only. Scope and contract/release bindings never change. PostgreSQL accepts only the exact pass-1/fence-1/zero-progress leased start, validates bounded leases using database time, requires every progress/status change to match one committed batch, and permits no-batch `STOPPED` only from a live exact fence with a same-transaction immutable stop Audit Event. One partial unique index permits one authoritative nonterminal run per company. Closed statuses are `ACTIVE`, `BLOCKED`, `BARRIER_REQUIRED`, `INCOMPATIBLE`, `STOPPED`, and `COMPLETED`; the current implementation cannot claim `COMPLETED` or `DRAIN_CLEAN`. |
| Approval Routing Backfill Batch | exact run/tenant/company scope, request ID, fence, pass and sequence, from/to cursor, exact scanned/applied/current/terminal/blocker counts, `has_more`, outcome, previous/current receipt hashes, database commit time | Append-only evidence for one bounded Serializable page. Closed durable outcomes are `CONTINUE`, `BLOCKED`, and `BARRIER_REQUIRED`. A deferred guard requires the batch, known blockers, receipt chain, cumulative counters, and run checkpoint/status to agree at commit. `RETRYABLE` and `INCOMPATIBLE` attempts write no batch. |
| Approval Routing Backfill Blocker Observation | exact run/batch/tenant/company/approval-instance scope, pass, bounded document-family text, closed stable blocker code, database observation time | Append-only durable evidence for a known deterministic blocker. The run/pass/instance/code identity is retry-safe. Cross-scope lineage is rejected. Unknown database, serialization, lease, or infrastructure failures are never stored as known blockers. |

Migration `20260727140000_approval_routing_backfill_orchestration` creates empty schema only: expected initial record counts are zero for all three relations and it performs no populated-data scan, routing conversion, audit synthesis, or source workflow mutation. The bounded scan uses the partial pending-instance index on `(tenant_id, company_id, created_at, id)`; UUID alone is not treated as insertion chronology.

For rehearsal, capture per-company pending Approval Instance counts, v0/v1 step counts by all 18 families, mixed-version/malformed rows, existing backfill audit counts, estimated batches `ceil(candidate_count / configured_batch_size)` plus reconciliation pages, query plans, lock/statement duration, and runtime ACLs. Verify exact retry, receipt-chain continuity, lease takeover/fencing, stale-holder rejection, blocker deduplication, page failure rollback, immutable batch/blocker denial, and paired backup/restore. Business reports and exports remain compatible because these relations are internal operational evidence and do not change approval outcomes, finance, inventory, or source statuses.

Lease and standalone orchestration evidence timestamps use `TIMESTAMPTZ(3)`. Cursor timestamps retain the source `ApprovalInstance.createdAt` `TIMESTAMP(3)` type so the direct partial `(tenant_id, company_id, created_at, id)` pending scan index remains usable; hosted database and maintenance sessions must enforce UTC. The current Option D grant posture gives the normal application runtime zero access to Run, Batch, and Blocker relations. Executor activation requires a separately reviewed least-privilege maintenance role and executable denial evidence.
...
Current Phase I scaffold implements approved-Purchase-Request quote capture with `quotation_request`, `supplier_quotation`, and `supplier_quotation_line` records scoped by tenant/company and selected location context. It records supplier, quote reference/date, currency from company configuration, quantity, UOM, unit price, line total, availability, lead time, terms, notes, and audit reason. Quote creation accepts a client-generated idempotency key and stores a SHA-256 request hash; exact retries by the same scoped actor return the original quote without a second audit event, while changed payloads, scope, or actor fail closed. It also highlights the lowest recorded cost and supports scoped quote CSV export. Formal supplier recommendation is recorded through `quotation_recommendation`, which stores the selected supplier quote, evaluated-total snapshot, quote count, selection reason, conditional non-lowest justification, conditional single-source or quote-shortfall justification, status, version, and evaluation snapshot. Recommendation creation consumes the configurable quotation threshold and minimum quote-count policy; requests at or above the threshold require the configured quote count or a documented shortfall justification. Recommendation submission creates a configurable approval instance for supplier-selection approval before PO conversion. Supplier quotation records may now link optional controlled evidence through source type `SUPPLIER_QUOTATION`; the shared attachment service enforces quote-management permission and purchase-request location scope, with scan/availability and download auditing. Mandatory quote-evidence requirements remain open pending the controlled-evidence matrix.

| Entity                   | Required fields                                                                                                                                                                                                                                                                              | Notes                                                                                                                                                                                                                                                                |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Supplier Quote           | `company_id`, `rfq_number`, `supplier_id`, `request_date`, `currency_code`, `status`, `subtotal_amount`, `tax_amount`, `discount_amount`, `freight_amount`, `other_charges_amount`, `supplier_accreditation_snapshot`, `idempotency_key`, `idempotency_request_hash` | Captures quote reference, validity, lead time, terms, commercial components, and supplier snapshot. Total is server-calculated in company currency. Optional controlled evidence links use source type `SUPPLIER_QUOTATION`; mandatory evidence remains policy-gated. |
...
**PO statuses:** `draft`, `pending_approval`, `approved`, `issued`, `amendment_pending`, `acknowledged`, `partially_received`, `fully_received`, `closed`, `cancelled`, `superseded`.
...
`DEC-0235` changes no stored PO or line monetary field. It suppresses only Overview rollups because all-status header/line aggregation, approval-rejected remainder, mixed currency, and live relation/snapshot behavior do not provide one authoritative commitment, exposure, or received-value basis. Source PO detail and authorized PO Status exports retain their documented row/lifecycle meanings and are not proof of a suppressed Overview aggregate.
...
| `idempotency_key`, `idempotency_request_hash`                                                       | Conditional / Yes for new create callers | Retry-safe draft creation. The key is unique within tenant/company, is retained with the receipt, and is never reused for a different actor, location, PO, or payload. Legacy rows may remain null. |
...
Current Phase I scaffold implements Receiving Report capture, posting, and full-document reversal from issued or partially received Purchase Orders. Draft receipts preserve PO, supplier, receiving location, receiver, external delivery reference, line snapshots, accepted/rejected/damaged/short quantities, discrepancy reason and evidence reference for discrepancy lines, destination inventory location, and audit history. New draft-create callers submit a bounded idempotency key; the server stores a canonical request hash and returns the original draft for an exact retry only after live authorization and actor/location/PO checks. Reusing a key for changed payload or scope is rejected without exposing the existing receipt ID. Keys are retained indefinitely and are not displayed as user-entered business data. Posting creates immutable `RECEIPT_IN` inventory movements only for accepted quantities, updates the balance cache, increments PO line received quantity, and moves the PO to `PARTIALLY_RECEIVED` or `FULLY_RECEIVED`. Reversal requires permission and reason, writes linked `REVERSAL` movements, restores PO received quantities/status, and keeps the original receipt and discrepancy history visible. Binary attachment upload, notification fanout, partial line reversal, and advanced inspection approvals remain deferred controlled transitions.
...
| Transfer Receipt      | `inventory_transfer_id`, `received_by_user_id`, `status`, `received_at`, `idempotency_key`, `idempotency_request_hash`, `posted_at`, `reversed_by_user_id`, `reversed_at`, `reversal_reason`, `discrepancy_flag`, `discrepancy_summary`                                                                                                                  | Durable receipt event for exact or partial destination receipt. New receipt callers bind a bounded tenant/company-scoped idempotency key and canonical lowercase SHA-256 request hash; legacy rows may remain `NULL`/`NULL`. The database coherence guard permits only both fields null or both present with a 64-character lowercase hexadecimal hash, rejects one-sided or malformed pairs, and performs no backfill or auto-repair. Same-key/same-hash replay is permitted only after live actor/location/transfer authorization; changed payload or scope fails closed. Posted receipt events can be reversed only as full events. |
...
| Operational Reason Code | `tenant_id`, `company_id`, `workflow`, `code`, `label`, `wastage_types`, `inventory_classes`, `requires_evidence`, `status`, `sort_order`                                                                                                                                                                                       | Configured dropdown source for Wastage, Stock Adjustment, and future exception classifications. For wastage, `DEC-0268` requires independent `wastage_types` and `inventory_classes` eligibility dimensions. Conservative F&B seed mappings are `SPOILAGE_EXPIRY` → `SPOILAGE_EXPIRY` / `FOOD`; `PREP_TRIM_LOSS` and `KITCHEN_ERROR` → `PREPARATION_LOSS` / `FOOD`; and `DAMAGED_PACKAGING` → `DAMAGE` / `FOOD, PACKAGING`. Each company reviews/configures its own rows; seeds do not replace evidence or approval policies. The legacy overloaded `applies_to` value is not authoritative for new wastage eligibility. |
...
`DEC-0077` adds a dormant, policy-empty qualification data boundary. `ControlledEvidencePolicyVersion` and `ControlledEvidencePolicyActivationEvent` are append-only company/action-scoped history; `ControlledEvidencePolicyActivation` is the compare-and-swap pointer to the exact current activation event. `ControlledEvidenceActionQualification` and `ControlledEvidenceActionSelection` preserve the exact policy, action/source and optional line, actor/session/approval lineage, selected controlled link, immutable object/scan version, canonical hashes, policy cardinality, and readiness snapshot used by one transaction. Database guards recompute authoritative canonical/hash/count relationships and revalidate mutable link and attachment readiness at commit. The runtime may read policy state and append qualification facts but cannot publish/activate policy or rewrite/delete history. The tables start empty and have no production publisher, adapter, call site, UI, seed, or default; PostgreSQL behavioral acceptance and `DEC-0047` policy approval remain pending.
...
Current implementation note: The bounded notification foundation creates scoped in-app approval-assignment notifications for implemented Phase I approval-backed workflows. An intermediate approval keeps the source record pending; in the same transaction it completes the current approval step, activates the next step, and re-evaluates current eligibility. In normalized next-step routing, a direct-user step creates at most one idempotent personal notification for its assigned user, while a role-scoped step creates no per-user notification rows and remains authoritative through live inbox eligibility. The five legacy initial procurement submission paths now use this same direct-user/role-scoped contract; no historical notification rows are rewritten. Purchase Request and Quotation Recommendation claims are protected by a scoped `DRAFT` status compare-and-set before approval creation, while Purchase Order balance-closure requests lock the authoritative order row before checking pending closures (`DEC-0078`). PostgreSQL parity and executable concurrent race evidence remain explicit activation gates. A final approve, return, or reject decision atomically creates the terminal outcome notification for the requester or responsible owner. Source-event keys and recipient uniqueness make applicable notification writes retry-safe. Notification read/archive state does not complete, approve, post, or otherwise mutate the source workflow. Scheduled reminder/escalation delivery, email or other external channels, preferences, and broader non-approval event fanout remain deferred.
...
- `PettyCashRequest` stores replenishment or disbursement requests against a fund with requested and terminal approved PHP amounts, lifecycle actors, purpose, justification, evidence reference, source event key, and no self-approval guardrails. While a normalized approval chain is active, `currentProposedAmountPhp` is the typed amount carried by that chain and `approvalProposalVersion` is its monotonic concurrency token. Submission initializes the current proposal from the requested amount. Each normalized current-step approve, return, or reject consumes the exact locked proposal/version through a Decimal compare-and-swap and increments the version once without changing the amount. Intermediate approval retains the proposal; terminal approve, return, or reject clears it while retaining the incremented version. Final approval writes the unchanged proposal to `approvedAmountPhp`. Amount changes remain blocked until Finance and Operations confirm a separate policy. Current service actions control create, submit, approve, return, reject, cancel, offline fulfill, void fulfillment, and close. Replenishment fulfillment increases the fund balance snapshot; disbursement fulfillment decreases it and is blocked if the fund would go negative. A fulfilled offline movement can be voided only through a controlled reversal ledger marker, required reason/evidence, `VOIDED` status, and audit history. The normalized writer remains feature-disabled and has no Payment Request, bank, journal, fund, ledger, disbursement, or settlement side effect.
- `PettyCashApprovalStepIntent` stores the immutable typed fact for one acted normalized petty-cash approval step. It records tenant/company/request/approval-instance/step lineage, step order, requested-amount snapshot, identical before/effective amounts, actor, optional normalized reason, consecutive proposal versions, decision-payload hash, idempotency key, and creation time. The server binds the typed approve/return/reject action, six-decimal amounts, normalized reason, and supplemental decision context into a stable canonical payload, derives a SHA-256 hash and versioned idempotency key, and appends the row in the same transaction as proposal CAS, approval/source outcome, audit, and permitted notification. PostgreSQL enforces one intent per approval step, one per instance/step order, scoped idempotency, valid amount/version bounds, active-chain lineage at insert, and append-only protection against update, delete, or truncate. Skipped, cancelled, failed, and unauthorized steps create no intent. The relation grants no amount-change or financial-posting authority, and its disposable-PostgreSQL behavioral specifications remain unexecuted while normalized routing is disabled.
...
- `PaymentRequest` stores controlled payment-preparation headers with company/location/supplier scope, PHP-only total, lifecycle status, requester/submitter/approver metadata, optional approval-instance link, reason/evidence, and idempotency key. Draft creation and final approval deterministically lock the referenced AP-invoice rows by ID and validate duplicate invoice references, exact tenant/company/location/supplier/currency coherence, line scope, positive requested amounts, header-to-line total equality, invoice-total and outstanding-capacity snapshots, and live capacity using fixed-precision decimal arithmetic. Final approval repeats these validations against locked live records and excludes only explicitly capacity-releasing request statuses from competing capacity. The exact `PAYMENT_READY`, match/tolerance, exception, invoice/request-status, and active-request eligibility matrix remains open; this validation neither settles AP nor releases or reserves bank funds.
- `PaymentRequestLine` stores AP-invoice payment request lines with requested amount, invoice total/outstanding-capacity snapshots, AP invoice link, line number, creator, and duplicated scope fields for fast authorization and integrity checks. Snapshot equality is revalidated against the locked invoice and live competing-request capacity at final approval; it is not an AP settlement balance.
...
Document numbering: `DocumentNumberSequence` stores the next value for a company, document type, and UTC year. Goods Receipt references use the `GOODS_RECEIPT` sequence and retain the existing company/reference uniqueness constraint. Migration backfill derives the next value from existing `RR-YYYY-#####` references without rewriting receipts.

Stock-count recount foundation: `StockCountAttempt` and `StockCountAttemptLine` are additive child records for immutable recount attempts. Existing sessions and lines are backfilled as attempt 1 with preserved IDs and source lineage. First-pass start, entry-save, submit, review, and cancellation transactionally mirror the legacy session/line path into attempt 1 during the reversible cutover; a failed attempt mirror rolls back the compatibility mutation. Scoped Stock Count reads and locks fail closed when `currentAttemptId` is missing. Detail, CSV, list, and paginated-page reads compare deterministic current-attempt and legacy header/lifecycle fields plus line digests using the canonical legacy-line lineage key and fail closed on mismatch. Additive database guards require current-attempt/session scope coherence and attempt-scoped adjustment header/line lineage. Terminal attempt evidence is guarded against update/delete; the only additional terminal exception is the exact controlled `SUBMITTED -> CANCELLED` transition, which requires a cancellation timestamp, nonblank reason, version increment of exactly one, and no evidence/scope/lineage change. It occurs in the same transaction that cancels the session and matching pending approval graph and creates no inventory movement. Operational reads still use the legacy compatibility projection. The dormant variance bridge records both session/legacy-line and current-attempt/attempt-line lineage, but generation remains disabled. The review-gated dashboard attempt aggregate is not exposed while Count Variance is inactive; production enablement still requires PostgreSQL plan/volume evidence, reviewed workflow cutover, and recovery gates.

`StockCountRecountTransition` is the append-only recovery lineage record. It binds one reviewed source attempt to one successor, the exact tenant/company/location/session scope, terminal adjustment disposition, reason/evidence narrative, actor/MFA session, scoped idempotency digest, and the exact active Stock Count review configuration revision/event/generation. It also requires a unique exact-scope `ControlledEvidenceActionQualification`; the database insert and deferred commit guards reject a missing, foreign, wrong-action, wrong-source/version, wrong-actor/session, or wrong-idempotency qualification. The qualification policy and action adapter remain dormant under `DEC-0077`, so the service fails closed before mutation and `evidenceReference` remains supplemental only. The transition does not duplicate attachment selection/version/checksum facts; those remain owned by immutable qualification selections once the separately approved evidence matrix and adapter exist.

Inventory Pilot approval-classification foundation: `InventoryPilotConfigurationRevision` stores one immutable, monotonically numbered, `SEALED` tenant/company cohort revision with schema version, optional exact predecessor revision/number/digest, canonical JSON, SHA-256 digest, source-decision reference, and sealing actor/time. Historic schema-v1 rows retain their original canonical bytes and cannot carry predecessor lineage. New `DEC-0273` seals are schema v2 and form a single-successor chain. `InventoryPilotEndpointMembership` normalizes exact `TRANSFER_SOURCE`, `TRANSFER_DESTINATION`, `COUNT_LOCATION`, and schema-v2 `OPENING_STOCK_LOCATION` membership to both an `InventoryLocation` and its parent `Location`; scoped composite foreign keys prove exact tenant/company/location lineage. `InventoryPilotItemMembership` normalizes the explicit same-scope high-risk Item-ID set. Sealed revision and membership history is append-only and owner-resistant through always-enabled database triggers. The migration creates no real draft, cohort, revision, activation, or stock rows.

`InventoryPilotConfigurationDraft` is the mutable schema-v2 authoring header. It stores tenant/company, `DRAFT` / `SEALED` / `ABANDONED` status, positive CAS version, exact optional predecessor identity/digest, creator and latest editor, terminal sealed-revision identity or abandonment actor/time/reason, and `DEC-0273` source reference. `InventoryPilotDraftEndpointMembership`, `InventoryPilotDraftItemMembership`, `InventoryPilotDraftParticipant`, and `InventoryPilotDraftRouteReadiness` normalize the included endpoint capabilities, explicit Item IDs, five named opening responsibilities (`PREPARER`, `SUBMITTER`, `OPERATIONS_REVIEWER`, `ACCOUNTING_REVIEWER`, `COMMAND_REQUESTER`) with exact user/role-assignment/role lineage, and one captured sealed Approval Rule definition/digest per family. Route rows keep that raw rule evidence separate from optional resolver evidence: `PurchaseRequest` requires a canonical resolver-evidence payload/digest, while every other family requires those fields to be null. Draft children may change only while their header remains `DRAFT`; headers and children cannot be hard-deleted, and terminal drafts cannot be reopened.

`InventoryPilotConfigurationSealOperation` is the immutable one-to-one draft/revision compilation record. It binds expected draft version, company-scoped idempotency key, request hash, exact sealed revision/number/digest, sealer, and evidence cutoff. One serializable seal transaction locks the company configuration sequence and draft, revalidates live permission, exact Company `MANAGE`, fresh MFA, creator/latest-editor separation, active endpoint/item/actor/route readiness, then creates the next schema-v2 revision and terminal draft state or creates none. Exact replay returns the original result; mismatched key reuse conflicts.

`InventoryPilotParticipantMembership` and `InventoryPilotRouteReadinessMembership` are immutable, digest-covered point-in-time evidence on the sealed revision. Participant membership contains exactly five distinct actors and their seal-time role-assignment evidence. Route readiness contains exactly one snapshot for each exact family: `PurchaseRequest`, `QuotationRecommendation`, `PurchaseOrder`, `InventoryTransfer`, `StockCountAttemptReview`, `WastageReport`, `StockAdjustment`, and `OpeningInventoryCutover`. Database guards recompute canonical/digest values, enforce exact five/eight cardinality and draft-to-sealed parity, and preserve append-only seal evidence. Named actors and route snapshots grant no permission, scope, approval, routing, command, executor, activation, or inventory authority; all live checks remain authoritative. The opening executor is not a participant responsibility and remains deployment-controlled.

The `PurchaseRequest` route-readiness payload has a fixed scenario context rather than certifying every Purchase Request route. Its raw rule canonical definition/digest remains independently database-verifiable. Its separate resolver-evidence canonical payload/digest records stable resolver ID `purchase_request_approval_rule_v1`, canonical input `isEmergency=false`, and the exact shared `resolvePurchaseRequestApprovalRule` outcome: the selected active/sealed rule is `DEFAULT`, route type is `normal`, and fallback is false, together with the same exact rule identity, lineage, version, definition, and definition digest. Live eligibility re-invokes the same production resolver with `isEmergency=false` and requires the same result. A valid `PR_EMERGENCY` rule may coexist but is outside this snapshot and is not emergency-route certification or UAT evidence.

Schema v1 is historic: existing cohorts, intents, and activation evidence remain pinned to their original revision/digest. Only the latest unsuperseded schema-v2 revision can be considered for a new Opening Inventory cohort, and the service must verify its exact canonical/digest shape and re-evaluate current actor scope/permissions and active sealed routes. A schema-v2 successor affects only future cohorts after the relevant controlled consumer selects it; sealing does not activate a family, repin an existing record, create an approval graph or command, or post inventory.

`InventoryPilotFamilyActivation` stores the current CAS generation for one `InventoryTransfer` or `StockCountAttemptReview` family, while `InventoryPilotFamilyActivationEvent` stores its append-only activation/deactivation history and prior-generation link. Events are serialized by a tenant/company-wide transaction advisory lock before the narrower family lock; concurrent families cannot establish different active revision/digest values. Active state must link exactly to an accepted event and a sealed configuration revision. Older revisions cannot be reactivated. Environment flags are denial-only kill switches and cannot create positive authority. No real activation state or event is seeded.

`InventoryTransferApprovalSubmissionIntent` and `StockCountReviewSubmissionIntent` are append-only typed replay/lineage records for normalized pilot approval submission while their family remains default-off. They bind the exact source, positive before/after source versions, canonical source/evidence and request hashes, scoped idempotency key, sealed configuration revision/digest, activation event/family/status/generation, submitter, and exact `ApprovalInstance`. Count review additionally binds the current `StockCountAttempt`, parent `StockCountSession`, and both attempt/session before/after versions. Database triggers require the source and pending approval graph to be in the submitted state at insertion. An exact key/request replay returns its original intent/approval result even after that graph later reaches a terminal decision; changed request or pinned authority facts conflict without mutation. A returned transfer uses a fresh key and a newly bound intent for a new cycle. These intents are approval evidence only and never create inventory movements or balances. The family-admission runtime path may `SELECT` and `INSERT` these two relations but cannot update or delete them; it reads sealed pilot configuration and activation state and cannot create activation authority. The separate server-authorized `DEC-0273` Setup Center service owns draft mutation and atomic seal compilation only; it does not activate a family or post inventory.
