...
- `PettyCashRequest` stores replenishment or disbursement requests against a fund with requested and terminal approved PHP amounts, lifecycle actors, purpose, justification, evidence reference, source event key, and no self-approval guardrails. While a normalized approval chain is active, `currentProposedAmountPhp` is the typed amount currently carried by that chain and `approvalProposalVersion` is its monotonic version. Submission initializes the current proposal from the requested amount. A final approval that omits an amount preserves the current proposal; the terminal `approvedAmountPhp` is written only when final approval completes, after which current proposal state is cleared. Intermediate approval with an amount different from the current proposal remains blocked until Finance and Operations confirm the amount-change policy. Current service actions control create, submit, approve, return, reject, cancel, offline fulfill, void fulfillment, and close. Replenishment fulfillment increases the fund balance snapshot; disbursement fulfillment decreases it and is blocked if the fund would go negative. A fulfilled offline movement can be voided only through a controlled reversal ledger marker, required reason/evidence, `VOIDED` status, and audit history.
- `PettyCashApprovalStepIntent` is the immutable typed schema foundation for one acted petty-cash approval step. It stores tenant/company/request/approval-instance/step lineage, step order, requested-amount snapshot, before and effective amounts, actor, optional reason, consecutive proposal versions, decision-payload hash, idempotency key, and creation time. PostgreSQL enforces one intent per approval step, one per instance/step order, scoped idempotency, valid amount/version bounds, active-chain lineage at insert, and append-only protection against update, delete, or truncate. The runtime writer and proposal-version compare-and-set remain pending while normalized routing is disabled; the relation does not itself grant amount-change authority.
