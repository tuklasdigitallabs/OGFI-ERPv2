...
Inventory Pilot approval-classification foundation: `InventoryPilotConfigurationRevision` stores one immutable, monotonically numbered, `SEALED` tenant/company cohort revision with canonical JSON, SHA-256 digest, source-decision reference, and sealing actor/time. `InventoryPilotEndpointMembership` normalizes exact `TRANSFER_SOURCE`, `TRANSFER_DESTINATION`, and `COUNT_LOCATION` membership to both an `InventoryLocation` and its parent `Location`; scoped composite foreign keys prove exact tenant/company/location lineage. `InventoryPilotItemMembership` normalizes exact same-scope `Item` membership. Sealed revision, endpoint, and item history is append-only and owner-resistant through always-enabled database triggers. No real cohort rows are seeded.
