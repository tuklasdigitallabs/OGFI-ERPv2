...
Stock-count recount foundation: `StockCountAttempt` and `StockCountAttemptLine` are additive child records for immutable recount attempts. Existing sessions and lines are backfilled as attempt 1 with preserved IDs and source lineage. First-pass start, entry-save, submit, and review transactionally mirror the legacy session/line path into attempt 1 during the reversible cutover. Scoped Stock Count reads and locks fail closed when `currentAttemptId` is missing. Detail, CSV, list, and paginated-page reads compare deterministic current-attempt and legacy line digests using the canonical legacy-line lineage key and fail closed on mismatch. Terminal attempt evidence is guarded against update/delete in the database; operational reads still use the legacy compatibility projection, while dashboard aggregates await a bounded attempt-only query contract and Count Variance remains inactive until all attempt-line parity, reviewed workflow cutover, and recovery gates are complete.
