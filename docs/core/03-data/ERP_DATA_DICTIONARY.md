...
Stock-count recount foundation: `StockCountAttempt` and `StockCountAttemptLine` are additive child records for immutable recount attempts. Existing sessions and lines are backfilled as attempt 1 with preserved IDs and source lineage. Terminal attempt evidence is guarded against update/delete in the database; operational services and Count Variance remain on the legacy path until the reviewed workflow cutover and recovery gates are complete.
