...
The current Operations Dashboard is a selected-location, read-only Phase I dashboard. It displays generated-at freshness, active company/brand/location scope, permission-gated KPI cards, assigned approval queue items, and source-linked exceptions from purchase requests, purchase orders, receiving, transfers, stock counts, wastage, stock adjustments, and inventory ledger reconciliation. It reuses server-side scoped services and `listPendingApprovals()` for approval visibility. The DEC-0036 `reporting.dashboard.unreconciled_mode` policy is enforced for reconciliation-dependent dashboard values: `block` suppresses the numeric ledger-variance card and exception rows and replaces them with a source-health warning linked to the visibly untrusted diagnostic profile, while `warn_and_link` and `show_only` keep the values visible with their configured trust posture. `DEC-0070` requires both balance-view and ledger-view permission and one selected-location, database-side variance predicate for the dashboard count/candidates, dedicated 25-row profile, search, and audited diagnostic CSV. The Overview trust-gate policy status is enrolled only for that same dual-permission population, is read-only control context, and does not expose policy metadata or a reconciliation destination to other dashboard users. The dashboard and profile do not repair balances or create adjustments. The dashboard does not provide all-location aggregation, project tracker metrics, attachment previews, stock posting, receiving, dispatch, approval, or other workflow mutation actions. The reconciliation CSV is an all-or-nothing selected-location diagnostic export: it uses the configured `reporting.export.max_rows` ceiling, fetches at most `maxRows + 1` rows in the same canonical statement, and returns `REPORT_EXPORT_ROW_LIMIT_EXCEEDED` / HTTP 413 with no partial file when the exact variance population exceeds the ceiling. Export audit metadata records the cap and whether search was applied, never raw search text.

`DEC-0235` suppresses Overview `PO commitment`, `Open PO exposure`, and `Received value`. No dashboard monetary report or export may derive from the removed all-status header/line rollups or label mixed-currency totals with one PO's currency. Existing PO Status and source exports remain authorized source-record reports, not reconciliation evidence for an Overview aggregate. A future signal requires one Decimal-safe, snapshot-consistent, relation-safe, single-currency closed dashboard/profile/page/export contract with exact lifecycle and cancellation-subtype semantics.
...
- The `zero-stock-v1` Inventory dashboard-profile CSV is a current selected-location, existing-balance-row export. It shares the dashboard/list/page predicate for active Inventory Location relation ownership and exact `qtyOnHand = 0`; positive, negative, inactive, malformed, and cross-scope rows are excluded, while multiple valid zero rows for one item remain separate rows. Optional normalized search may only narrow the population. Deterministic ordering ends in balance ID. The export reauthorizes existing export permission, applies `reporting.export.max_rows` through exact count preflight and a `maxRows + 1` race guard, and returns truthful `413` with no partial file when either limit check fails. Audit metadata is aggregate-only and excludes search text, row payloads, identifiers, and internal errors.
- The `lot-expiry-data-v1` Inventory dashboard-profile CSV is a current selected-location, existing-balance-row data-presence export. It shares one static parameterized dashboard/list/page predicate for active Inventory Location and related-record ownership plus trimmed nonblank lot or recorded expiry; all quantity signs are included. Optional normalized search may only narrow the population, and deterministic ordering ends in balance ID. The export reauthorizes existing permission, applies `reporting.export.max_rows` through exact count preflight and a `maxRows + 1` race guard, and returns truthful `413` with no partial file. Audit metadata is aggregate-only and excludes search text, lot/expiry values, row payloads, identifiers, and internal errors. The export is not tracking-compliance, coverage, accountability, traceability, or historical-snapshot evidence.
- Ordinary Stock Balances CSV uses the same selected-location predicate and deterministic balance-row ordering, applies the configured `reporting.export.max_rows` ceiling with a `maxRows + 1` database guard, and fails with `REPORT_EXPORT_ROW_LIMIT_EXCEEDED` / HTTP 413 rather than silently exporting a partial population. Its export audit metadata records only the cap and whether search was applied.
- Inventory Ledger CSV uses immutable movement-row grain and the same selected-location/tenant/company scope as the ledger source. Its export-only cap is distinct from the UI list bound; overflow fails with `REPORT_EXPORT_ROW_LIMIT_EXCEEDED` / HTTP 413 instead of presenting a UI-truncated result as a complete ledger export.
- Wastage and Stock Adjustment CSVs use document-summary row grain. Each applies the configured synchronous cap before mapping rows, returns HTTP 413 with no partial file on overflow, and records only aggregate cap/profile metadata in export audit events.
- Receiving CSVs use one emitted receiving-line row for ordinary exports and one receipt row for the follow-up profile; Transfer CSVs use emitted transfer/receipt-line rows; Stock Count CSVs use emitted count-line rows. Each route applies the configured cap before completing the CSV, maps overflow to HTTP 413 with no partial file, and keeps audit metadata aggregate-only.
- Purchase Request and Purchase Order register CSVs use one exported document row per source record, while Supplier Quote CSV uses its emitted quote-line rows. Their export adapters apply the configured synchronous cap and the same no-partial-file overflow contract; Purchase Request/Order audit events persist only filter-presence flags and safe policy metadata, not raw filters or caught error text.
- No `Updated this week` or recent-inventory-event report/export may be derived from `InventoryBalance.updatedAt`. That field is mutable balance-cache lifecycle metadata, not authoritative event time. `DEC-0234` suppresses the Overview signal and generic destination; a future immutable Inventory Movement report/profile requires a separately confirmed timestamp, period/week/timezone, grain, permission, export, and audit contract.
