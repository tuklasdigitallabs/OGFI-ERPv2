...
| SPF-004 | Authorization regression gate | Verify tenant, company, brand, location, department, project membership, restricted-project, and direct-route/API enforcement | In progress; final local candidate gates and independent re-review pass, while hosted exact-SHA CI evidence remains pending |
...
- Confirmed `DEC-0041`, `DEC-0042`, and `DEC-0043`: executable database-backed boundary coverage is required for high-risk authorization surfaces; workforce access is location-primary; and tenant-global role administration requires explicit `core.tenant_role_administer` plus selected-company target eligibility.
...
- Hosted run `29824012949` for implementation SHA `020c334fa692d2e8a2957df5e7705918fb90a02e` passed migration, seed, secret, release-tool, lint, typecheck, build, package-test, and authorization steps, then reached the Playwright 60-second ceiling during E2E; manifest generation was consequently skipped. Local expansion startup already consumes 45–49 seconds, so the global test and web-server ceilings were raised to 120 seconds while preserving 15-second assertion waits and the explicit 360-second long-workflow cap. A clean exact-SHA rerun is required before closure.
...
- Remaining closure step: commit/push the reviewed candidate and accept the hosted exact-SHA CI artifact before marking SPF-004 complete.
