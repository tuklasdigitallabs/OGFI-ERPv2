...
| SPF-006 | Audit and activity integrity | Verify important create, update, transition, approve, post, reverse, cancel, archive, upload, download, export, and denied actions produce the required immutable history; bound or aggregate repeated denied-event writes so enumeration attempts cannot amplify audit storage without limit | In progress; approval/admin/wastage/receiving/stock-adjustment concurrency checkpoint independently accepted and fully regressed, while database append-only enforcement, denial and fanout bounding, remaining request/reversal and audit-semantic gaps, broader behavioral races, and the cross-workspace audit matrix remain pending |
...
- SPF-006 remains open. `DEC-0049` database-enforced append-only protection and restricted production database-role separation, bounded or aggregated authorization-denial recording, an approved bound for role-assigned approval fanout, canonical locking for the PO balance-closure request path, the Stock Adjustment posting MFA audit-label correction, executable intermediate/final/return/reject coverage for all seven migrated approval workflows, real service-to-service race coverage, the receiving reference allocator gap, and the cross-workspace audit/action matrix remain required before promotion.
