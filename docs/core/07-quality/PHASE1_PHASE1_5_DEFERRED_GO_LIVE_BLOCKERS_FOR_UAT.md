...
| DGB-012 | Controlled evidence uploads and storage segregation | Complete the shared hybrid evidence model: retain structured notes/external references, add private uploads where policy requires evidence, use the environment-isolated same-Hostinger-VPS broker selected by `DEC-0046`, opaque immutable keys/versions, source-record and restricted-project authorization, quarantine and private ClamAV scanning, checksum verification, audited downloads, preservation/legal hold, and independently recoverable encrypted backup. Text alone must not satisfy high-risk evidence requirements unless policy explicitly permits a verified external reference. | Product Owner / IT-Security Owner / DevOps Owner / Data-Privacy Owner | Approved host path, quota/high-water values, key custody, cross-scope denial tests, upload/scan/download/archive/hold audit evidence, retention policy, independent backup and paired restore proof, and workspace attachment requirement matrix | Shared production-foundation blocker before workspace production certification; external storage migration is deferred until a documented trigger |
