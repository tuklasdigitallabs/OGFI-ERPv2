...
Use this guide to collect generated release artifacts, signed evidence documents, and external-security proof references into one review folder before running the final GO / NO-GO evidence summary.
...
| Release readiness register export | `DATABASE_URL=<pilot-or-staging-url> pnpm release:readiness-register` after readiness gates, UAT/deployment/enablement evidence, or Release Board decisions change; the command writes a checksum sidecar for final evidence integrity. This export uses `psql` when available and falls back to Prisma when PostgreSQL client tools are not installed. | `release-readiness-register/release-readiness-register-*.csv` and `release-readiness-register/release-readiness-register-*.csv.sha256` |
...
| Pilot readiness preflight    | `pnpm release:pilot-readiness-preflight` before the DB-backed readiness check; for final release review rerun with `PILOT_REQUIRE_RELEASE_GATES_READY=true` | `pilot-readiness/pilot-readiness-preflight-*.txt`         |
| Pilot setup readiness        | `DATABASE_URL=<pilot-or-staging-url> pnpm release:pilot-readiness`; for final release review use `DATABASE_URL=<pilot-or-staging-url> PILOT_REQUIRE_RELEASE_GATES_READY=true pnpm release:pilot-readiness` so the report proves accepted release readiness gates, not just setup data | `pilot-readiness/pilot-readiness-*.txt`                  |
...
| Evidence manifest            | `pnpm release:evidence:manifest` after each evidence bundle or final collected folder is complete; the command also writes a checksum sidecar that final review verifies | `manifests/release-evidence-manifest-*.txt` and `manifests/release-evidence-manifest-*.txt.sha256` |
...
| Final-review readiness status | `pnpm release:final-review-status` after collecting or updating final review evidence, including signed evidence documents and external-security proof references; this reports blockers but does not approve release | `final-review-status/final-review-status-*.txt`          |
...
| Signed evidence checklist    | `pnpm release:signed-evidence-checklist` to generate the owner-facing signed document, external-security proof, override path, evidence-run, decision-field, and manifest-integrity checklist; this is advisory and does not create or approve signed documents | `signed-evidence-checklist/signed-evidence-checklist-*.txt` |
...
| Rollback summary             | **Unavailable for release credit:** legacy rollback execution is disabled; future DEC-0248 same-fence rollback must generate this artifact | `staging-rollback/rollback-summary.txt`                  |
| Post-rollback smoke          | **Unavailable for release credit:** must be generated by the future DEC-0248 recovery/rollback path against the authoritative cutover target | `staging-rollback/smoke/smoke-*.txt`                     |
...
10. Run `pnpm release:metadata-worksheet` and `pnpm release:metadata-env-template` to capture the release metadata gaps and generate a reusable non-secret shell template for one approved evidence session. Approved release metadata may be stored in `.env.release.local` or a command-scoped `LOCAL_ENV_FILES` file so evidence commands can reuse the same values without repeated shell exports; explicit environment variables still take priority. Do not store secrets, database passwords, or raw credentials in these metadata files. After owners provide approved metadata, run `pnpm release:metadata-session-lock` with the same environment values to preserve the final `RELEASE_EVIDENCE_RUN_ID` and copy-ready command sequence for all evidence owners.
...
12. Run the staging release rehearsal workflow for the release candidate, or run `pnpm release:summary` with approved release metadata when collecting evidence manually. Set `RELEASE_EVIDENCE_RUN_ID`; final review requires one shared evidence session across release summary, pilot readiness, signed documents, external-security proof references, manifest, final review, and GO / NO-GO artifacts.
...
14. Record rollback evidence as blocked. The legacy staging rollback workflow and command intentionally exit `78`; a manually authored summary is not execution evidence.
15. After the DEC-0248 same-fence rollback/recovery path is implemented and accepted, collect its rollback summary and authoritative post-rollback smoke files into `staging-rollback/`.
...
23. Run `pnpm release:interim-review` when the team needs a local technical-review artifact before external evidence collection is complete. The report is labeled `EVIDENCE_TIER | LOCAL` and may support client walkthrough preparation, but it does not replace pilot UAT, database restore proof, rollback proof, training evidence, signed documents, external-security proof references, final-review status, or GO / NO-GO.
...
28. Copy signed evidence documents into `signed-documents/` or record their approved external paths. Copy approved external security proof references into `external-security/`; the files must start with `mfa-provider-enrollment-and-runtime-proof.`, `idp-session-invalidation-proof.`, `vault-or-artifact-storage-index.`, and `break-glass-review-and-revocation-proof.`.
29. Run `pnpm release:signed-evidence-status` after copying signed evidence documents, external security proof references, or setting `RELEASE_*_EVIDENCE_FILE` overrides. A BLOCKED result is expected until approved signed documents exist, have no unresolved placeholders, include the matching `Evidence run ID`, and include non-placeholder signoff fields for `Signed by`, `Role`, `Date`, `Decision`, and `Owner`. The signoff date must include `YYYY-MM-DD` or `MM/DD/YYYY`, and the decision must be an explicit release decision such as approved, GO, conditional GO, hold, no-go, rollback, or deferred. Each external security proof must include the same `Evidence run ID` and the marker `RESULT | PASS | External security proof captured.`.
...
Valid override variables are the `PILOT_MIN_*` values checked by `scripts/pilot-readiness-preflight.mjs` and `scripts/pilot-readiness-check.mjs`. A lower threshold is allowed only when the pilot scope intentionally excludes a scenario and the exclusion is recorded as an approved waiver or deferral in the UAT evidence pack. For release rehearsal, final review, and GO / NO-GO, set `PILOT_REQUIRE_RELEASE_GATES_READY=true`; this strict mode fails the DB-backed readiness report if required ERP release gates are missing, pending, held, accepted without evidence, conditionally/waived without decision notes, or when the GO / NO-GO gate status does not match the latest Release Board decision. The strict report must also show the live security rows for `Pending controlled access requests`, `Privileged users missing verified MFA evidence`, `Pending provider session invalidations`, and `Break-glass access open or post-review due`, all at zero before final approval. `pnpm release:status-suite:strict` treats `RESULT | READY FOR GO / NO-GO`, `RESULT | GO REVIEW READY`, and `RESULT | CONDITIONAL GO REVIEW` as valid non-blocking terminal states; `NO-GO`, `BLOCKED`, missing evidence, and command failures remain blocking.
...
The status suite intentionally does not run `pnpm release:evidence:manifest`. Run the suite while triaging blockers, finish or copy the real source evidence, signed documents, and external-security proof references, then run `pnpm release:evidence:manifest` as the final collection step before rerunning `pnpm release:final-review-status` and `pnpm release:go-no-go`.
...
| `external-security/` | Security Owner / IT Owner / Release Manager |
...
This status report is advisory. It must remain blocked until real release metadata, staging evidence, rollback proof, signed evidence documents, external-security proof references, and a fresh manifest are present.

To check signed evidence documents and external-security proof references before regenerating the final manifest, run:
...
The GO / NO-GO summary directly checks the signed UAT, deployment/rollback, and training evidence documents, then also requires `manifests/release-evidence-manifest-*.txt` plus the matching `.sha256` sidecar generated by `pnpm release:evidence:manifest`. The manifest records SHA-256 checksums for collected evidence files, and final review verifies the manifest sidecar so reviewers can detect missing, changed, or tampered artifacts during handoff.
The browser **Export Readiness Register** action includes a paired **Download SHA-256** action and an `X-OGFI-CSV-SHA256` response header for review packets. Download the CSV and SHA-256 file from the same page view so their generated timestamp matches. For final release evidence, still run `DATABASE_URL=<pilot-or-staging-url> pnpm release:readiness-register` so the collected evidence folder contains the DB-backed CSV and its `.csv.sha256` sidecar. The CLI export can run without `psql`; when PostgreSQL client tools are unavailable it uses the Prisma client against the supplied `DATABASE_URL`.
Generate the manifest after collecting build, security, snapshot preflight, migration, backup, restore, release-readiness register export, pilot-readiness, smoke, rollback, signed-document evidence, external-security proof references, and the latest passing focused status reports. Final review and GO / NO-GO require passing `data-snapshot-status`, `backup-restore-status`, `uat-status`, `pilot-uat-status`, `enablement-status`, and `signed-evidence-status` artifacts. They also verify that release summary, migration snapshots, backup/restore, rollback, release-readiness register export, pilot readiness, UAT status, enablement status, and external-security proof references share one non-placeholder `RELEASE_EVIDENCE_RUN_ID`; a missing, placeholder, stale, or mixed-session evidence package will keep the final report at NO-GO. The evidence-run consistency check also rejects setup-only pilot readiness artifacts that lack `PILOT_REQUIRE_RELEASE_GATES_READY=true`, `requireReleaseGatesReady=true`, and `DEC-0036 strict release gate status`. When `RELEASE_EVIDENCE_RUN_ID` is set for the final command, every required artifact must match that exact approved value.
