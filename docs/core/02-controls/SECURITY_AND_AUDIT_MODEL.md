...
- Item option-catalog lookup uses separate exact Caddy global/source zones and a process-local, non-queuing in-flight gate after session, bounded input, and server authorization but before catalog database queries. Unauthenticated, malformed, or denied traffic cannot consume catalog-work permits. Its private token-protected health route is denied at the public Caddy listener and exposes fixed-cardinality aggregate outcomes and kinds only through the loopback bridge; query text, selected IDs, users, source addresses, tenants, and companies are prohibited. Authentication monitoring parses only its exact zones so lookup traffic cannot alter authentication alerts. Production limits remain blocked until hosted load, shared-NAT, restart, alert, and rollback evidence is reviewed.
...
- Local controlled-UAT account provisioning may use an administrator-issued, one-use temporary password only for an active target with no administrator, approver, sensitive role, or high-risk scope. The credential is Argon2id-hashed, expires after 30 minutes, is consumed on first sign-in, forces a server-enforced password change before normal routes, invalidates prior sessions and the privilege epoch, and records no raw secret. Self-issue, cross-company targets, privileged targets, and high-risk locations fail closed. This is a time-bounded SMTP-free UAT exception, not production recovery authority.
...
- Dormant controlled-evidence policy versions, activation events, qualifications, selections, and scan attempts are also append-only under `DEC-0077`. A scoped mutable pointer may advance only through compare-and-swap to an immutable activation event; runtime cannot publish or activate policy. Database guards bind canonical payloads, hashes, policy cardinality, source/actor/session/evidence lineage, and commit-time link/readiness state. The adapter registry is empty and runtime qualification is constant-disabled, so these controls authorize no current business action.
- The dormant `DEC-0247` producer-barrier generation and provenance relations authorize no evidence creation. Statement-level `ENABLE ALWAYS` triggers reject every insert, including owner/replication-role attempts; separate always-enabled guards reject update, delete, and truncate. Runtime and `PUBLIC` retain zero table/column privileges, and runtime may execute only the reviewed company-scoped shared-lock routine. A later governed migration must replace the insert denial together with the closed typed writer perimeter before any provenance fact can exist.
- The `DEC-0247` `approval_shadow` schema is private diagnostic infrastructure, not an authorization or audit surface. Its 18 fixed-family `SECURITY INVOKER`, `STABLE` observers expose only a binary structural match/miss to the controlled migrator; `PUBLIC` and the ordinary runtime have neither schema usage nor function execution. The routines use `search_path=pg_catalog`, public-qualified relations, no dynamic SQL, no DML, and no explicit advisory/row/table lock. Controlled verification now attests each live routine's exact signature, owner, SQL language, volatility, invoker mode, parallel mode, search path, null SQL-body representation, and SHA-256 source body. The owner/migrator graph also fails closed unless its only touching edge is migrator-to-owner with `SET` true and `ADMIN`/inherit false. A positive result is not routing, policy, approval, readiness, certification, or activation evidence. Any future grant or application integration still requires a new security decision, invocation audit design, populated hosted restore/performance proof, and exact-candidate review.
...
- The existing Inventory Pilot family-admission path remains read-only toward sealed configuration and activation state and may only append its family-specific typed submission intents; it cannot call the revision canonicalizer or create activation authority. The separate `DEC-0273` Setup Center authoring/seal service does not broaden transfer/count admission or activation rights.
- Inventory Pilot configuration authoring under `DEC-0273` is confined to the server-owned Setup Center service. View, draft, and seal actions require their dedicated permission plus a live/effective role assignment, active session and privilege epoch, and exact selected-company `MANAGE`; seal also requires fresh MFA, a reason, and an actor who is neither the draft creator nor latest editor. Baseline permission grants are limited to the configured ERP Administrator (`CONFIGURED_ADMIN`) and System Super User (`CONFIGURED_SUPER_USER`) roles, but role names are never authority. Cross-tenant/company records and unavailable details fail closed without enumeration.
- Mutable `InventoryPilotConfigurationDraft` headers and normalized endpoint, item, participant, and route-readiness children may change only while the header is `DRAFT`; version CAS and audit events protect updates, and abandonment preserves history. Sealing takes a tenant/company advisory lock plus the exact draft row lock, rechecks authority/readiness after locking, and uses serializable atomic creation of the next immutable schema-v2 revision, its normalized memberships, the seal operation, terminal draft state, and audit event. Idempotency-key/request-hash replay is exact; conflicts and failed readiness create no partial revision.
- Sealed Inventory Pilot revisions and memberships remain immutable control-plane evidence. Schema-v2 canonical bytes and SHA-256 digest cover predecessor lineage, exact endpoint capabilities and selected Item IDs, five distinct named participant/role-assignment snapshots, all eight exact route-readiness snapshots, sealing actor/time, and evidence cutoff. Always-enabled guards validate route snapshot digests, draft/compiled parity, exact five/eight cardinality, terminal draft/seal coherence, and reject hard deletion; seal operations and sealed participant/readiness memberships are append-only. These snapshots are point-in-time evidence, not current authorization, routing, activation, approval, opening-command, or stock authority.
- The one `PurchaseRequest` readiness snapshot is context-bound to the standard non-emergency pilot scenario. Canonical evidence persists resolver ID `purchase_request_approval_rule_v1`, invokes the shared production `resolvePurchaseRequestApprovalRule` contract with `isEmergency=false`, and accepts only the exact selected active/sealed `DEFAULT` rule with `routeType=normal` and `fallbackUsed=false`. Live eligibility rederives and compares the same resolver input/outcome; missing/default drift, fallback, or substitution fails closed. A valid `PR_EMERGENCY` route may coexist but is not certified and grants no emergency UAT or release credit.
- Schema-v1 revisions retain their historic canonical bytes and remain available only for already pinned history and the previously defined classifier/activation boundary. New Setup Center seals are schema v2. The Opening Inventory creation service considers only the latest unsuperseded schema-v2 revision, verifies its canonical/digest and exact evidence shape, and re-evaluates current participant permissions/scopes and active sealed routes before offering or accepting a new cohort. A successor can govern only future cohorts through the separately controlled consumer path; existing cohorts and intents remain pinned. Sealing itself changes no activation, source status, approval graph, command, inventory movement, balance, custody, or financial record.
- The amended `DEC-0248` prohibits standalone hosted migration, direct wrappers, SSH-held release orchestration, and deploy-user `sudo`. The legacy migration unit, combined-credential verifier, staging deploy/rollback scripts, workflow jobs, and direct controlled-migration CLI fail closed. The migrator-only library requires an injected trusted runner and exposes no executable hosted path; runtime/application identity and append-only checks remain a future root-installed split-verifier boundary. No hosted database change is authorized until one root-owned release service/cgroup, immutable digest/SHA-bound artifact, split credentials, fsync-safe phase journal, boot recovery, authoritative served-SHA smoke, and same-fence rollback are implemented and accepted. A second executor/host/namespace/shared credential still requires a database session keeper. This is official-path serialization, not database atomicity or root/migrator compromise containment.
