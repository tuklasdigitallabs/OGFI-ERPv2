...
- Inventory Pilot revisions, endpoint/item memberships, activation events, and activation state are runtime read-only control-plane evidence. Runtime can only read and append the family-specific typed submission intents and cannot call the revision-reading canonicalizer. Exact function hashes/properties, fixed search paths, ACLs, and always-enabled trigger semantics are part of the database role contract; a structurally valid row written by an unauthorized credential is never accepted as configuration authority.
