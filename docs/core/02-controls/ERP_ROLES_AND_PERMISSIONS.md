...
| Ledger Variance profile | The cache-to-ledger reconciliation profile, its diagnostic export, and an exact reconciliation trace require both `inventory.balance.view` and `inventory.ledger.view`. Neither permission alone grants the combined comparison. Ordinary Inventory Ledger browsing remains ledger-view only; every trace remains read-only. |
| Dashboard trust status | The Overview trust-gate policy status is a read-only control observation for users who hold both `inventory.balance.view` and `inventory.ledger.view`; users without the complete Ledger Variance entitlement receive no trust-mode, override, decision, observation, or reconciliation destination from the dashboard. |
