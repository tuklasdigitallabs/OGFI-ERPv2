...
## Post-acceptance hardening — 2026-09-21

- Status: COMPLETE for the approved visual modernization pass on the implementation branch `ui-modernization-foundation`.
- Shared behavior: theme selection now supports Light, Dark, and System modes with OS preference tracking; reduced-motion users receive transition-free UI; the shell header and content spacing are tightened without changing routes or authorization.
- Operational workspaces: Projects uses URL-backed Overview, Members, Risks, and Activity tabs with server-side pagination; Work Boards exposes a mobile task list while retaining the desktop board; Finance source queues are paginated instead of capped to the first five records.
- Validation: web lint and typecheck passed; focused Vitest coverage passed for dashboard, receiving, inventory, inventory ledger, project tasks, and finance services. Browser checks passed on Chromium desktop and Pixel mobile after recreating the disposable local PostgreSQL volume and applying the existing migrations and demo seed: receiving/procurement 6/6, approvals/inventory 10/10, Projects/Work Boards/Finance 2/2, and dashboard responsive light/dark verification 2/2. The browser run also confirmed no horizontal overflow on the covered workspaces.
- Test maintenance: the dashboard browser assertion was aligned with the current visible dashboard headings and now verifies Light/Dark mode persistence and HTML theme application; a focused workspace sweep covers Projects, Work Boards, and General Ledger.
- Environment note: the checked-in schema still contains the pre-existing overlong `InventoryLowStockThreshold` index map name; local browser QA used a temporary reverted name only to regenerate the Windows test client. No schema file or migration was committed.
- Recovery: the pre-change restore point remains available at branch `restore-point-before-ui-modernization` (commit `cca08033`) and archive `restore-points/ogfi-erp-before-ui-modernization-2026-09-21.tar.gz`.
