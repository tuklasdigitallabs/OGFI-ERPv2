...
2. Open administration views for companies, roles, permissions, approval rules, and audit events.
   In Role Detail, use the permission search and Sensitive / Overrides / Recommended Drift filters. Demonstrate that unsaved choices remain selected while moving between pages and filters, that only `Save Permission Overrides` commits them, and that Cancel or a full refresh discards the browser-memory draft.
