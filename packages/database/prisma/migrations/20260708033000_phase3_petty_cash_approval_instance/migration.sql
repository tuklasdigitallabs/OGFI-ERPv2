...
CREATE UNIQUE INDEX "PettyCashRequest_approvalInstanceId_key"
  ON "PettyCashRequest"("approvalInstanceId");

ALTER TABLE "PettyCashRequest"
  ADD CONSTRAINT "PettyCashRequest_approvalInstanceId_fkey"
...
SELECT
  ...
  100,
  true,
  '{"source":"phase3_petty_cash_approval_instance","appliesTo":"non_posting_petty_cash_request_approval"}'::jsonb
FROM "Company" c
WHERE NOT EXISTS (
  SELECT 1
  ...
  WHERE ar."tenantId" = c."tenantId"
    ...
    AND ar."transactionType" = 'PettyCashRequest'
...
WHERE ar."transactionType" = 'PettyCashRequest'
  AND ar."isActive" = true
  AND NOT EXISTS (
    SELECT 1
