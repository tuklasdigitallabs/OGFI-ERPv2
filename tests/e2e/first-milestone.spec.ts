...
test("first milestone purchase request path works end to end", async ({
  ...
  await page.getByRole("button", { name: "Open supplier", exact: true }).click();
  const createdSupplierWorkspace = page.getByRole("dialog", { name: `${marker} Supplier` });
  await expect(createdSupplierWorkspace).toBeVisible();
  await createdSupplierWorkspace.getByRole("link", { name: "Accreditation & lifecycle" }).click();
  await createdSupplierWorkspace.getByRole("link", { name: "Open supplier deactivation controls" }).click();
  await createdSupplierWorkspace.getByLabel("Deactivation reason", { exact: true }).fill(`E2E supplier deactivation for ${marker}`);
  await createdSupplierWorkspace.getByRole("button", { name: "Deactivate supplier", exact: true }).click();
  ...
  await seededSupplierRow.click();
  await page.getByRole("button", { name: "Open supplier", exact: true }).click();
  const seededSupplierWorkspace = page.getByRole("dialog", { name: seededSupplierDisplayName });
  await expect(seededSupplierWorkspace).toBeVisible();
  await seededSupplierWorkspace.getByRole("link", { name: "Catalog", exact: true }).click();
  await seededSupplierWorkspace.getByRole("link", { name: "Create supplier-item link" }).click();
